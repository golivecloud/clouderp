...
# Utils
#----------------------------------------------------------
execfile(join(dirname(__file__), '..', 'gce', 'release.py'))
...
def publish(o, type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/clouderp_*.%s" % (o.build_dir, extension))[0]
        published.append(_publish(o, release))
    return published

class clouderpDocker(object):
    def __init__(self):
...
class KVM(object):
    def __init__(self, o, image, ssh_key='', login='gce'):
        self.o = o
        self.image = image
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setuppath = glob("%s/gce-server-setup-*.exe" % self.o.build_dir)[0]
        setupfile = setuppath.split('/')[-1]
        setupversion = setupfile.split('gce-server-setup-')[1].split('.exe')[0]

        self.rsync('"%s" %s@127.0.0.1:' % (setuppath, self.login))
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"clouderp %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('/cygdrive/c/"Program Files"/"clouderp %s"/server/gce-server.exe -d mycompany -i base --stop-after-init' % setupversion)
        self.ssh('net start clouderp-server-%s' % version)
...
def _prepare_build_dir(o, win32=False):
    ...
    system(cmd + ['%s/' % o.clouderp_dir, o.build_dir])
    try:
        for addon_path in glob(join(o.build_dir, 'addons/*')):
            if addon_path.split(os.path.sep)[-1] not in ADDONS_NOT_TO_PUBLISH:
                shutil.move(addon_path, join(o.build_dir, 'gce/addons'))
    except shutil.Error:
        # Thrown when the add-on is already in gce/addons (if _prepare_build_dir
        # has already been called once)
        pass

def build_tgz(o):
    system(['python2', 'setup.py', 'sdist', '--quiet', '--formats=gztar,zip'], o.build_dir)
    system(['mv', glob('%s/dist/clouderp-*.tar.gz' % o.build_dir)[0], '%s/clouderp_%s.%s.tar.gz' % (o.build_dir, version, timestamp)])
    system(['mv', glob('%s/dist/clouderp-*.zip' % o.build_dir)[0], '%s/clouderp_%s.%s.zip' % (o.build_dir, version, timestamp)])

def build_deb(o):
    # Append timestamp to version for the .dsc to refer the right .tar.gz
    cmd=['sed', '-i', '1s/^.*$/clouderp (%s.%s) stable; urgency=low/'%(version,timestamp), 'debian/changelog']
    ...
    system(['mv', glob('%s/../clouderp_*.deb' % o.build_dir)[0], '%s' % o.build_dir])
    system(['mv', glob('%s/../clouderp_*.dsc' % o.build_dir)[0], '%s' % o.build_dir])
    system(['mv', glob('%s/../clouderp_*_amd64.changes' % o.build_dir)[0], '%s' % o.build_dir])
    system(['mv', glob('%s/../clouderp_*.tar.gz' % o.build_dir)[0], '%s' % o.build_dir])

def build_rpm(o):
    system(['python2', 'setup.py', '--quiet', 'bdist_rpm'], o.build_dir)
    system(['mv', glob('%s/dist/clouderp-*.noarch.rpm' % o.build_dir)[0], '%s/clouderp_%s.%s.noarch.rpm' % (o.build_dir, version, timestamp)])

def build_exe(o):
    KVMWinBuildExe(o, o.vm_winxp_image, o.vm_winxp_ssh_key, o.vm_winxp_login).start()
    system(['cp', glob('%s/gce*.exe' % o.build_dir)[0], '%s/clouderp_%s.%s.exe' % (o.build_dir, version, timestamp)])
...
def _prepare_testing(o):
    if not o.no_tarball:
        ...
        subprocess.call(["cp", "package.dfsrc", os.path.join(o.build_dir, "docker_src", "Dockerfile")],
                        cwd=os.path.join(o.clouderp_dir, "setup"))
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_src")],
                        cwd=os.path.join(o.clouderp_dir))
        subprocess.call(["docker", "build", "-t", "clouderp-%s-src-nightly-tests" % version, "."],
    ...
    if not o.no_debian:
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_debian")],
                        cwd=os.path.join(o.clouderp_dir))
        subprocess.call(["docker", "build", "-t", "clouderp-%s-debian-nightly-tests" % version, "."],
    ...
    if not o.no_rpm:
        ...
        subprocess.call(["cp", "package.dfcentos", os.path.join(o.build_dir, "docker_centos", "Dockerfile")],
                        cwd=os.path.join(o.clouderp_dir, "setup"))
        subprocess.call(["docker", "build", "-t", "clouderp-%s-centos-nightly-tests" % version, "."],
                        cwd=os.path.join(o.build_dir, "docker_centos"))

def test_tgz(o):
    with docker('clouderp-%s-src-nightly-tests' % version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.tar.gz'
        wheezy.system("service postgresql start")
        wheezy.system('pip install /opt/release/%s' % wheezy.release)
        wheezy.system("useradd --system --no-create-home clouderp")
        wheezy.system('su postgres -s /bin/bash -c "createuser -s clouderp"')
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('mkdir /var/lib/clouderp')
        wheezy.system('chown clouderp:clouderp /var/lib/clouderp')
        wheezy.system('su clouderp -s /bin/bash -c "clouderp.py --addons-path=/usr/local/lib/python2.7/dist-packages/gce/addons -d mycompany -i base --stop-after-init"')
        wheezy.system('su clouderp -s /bin/bash -c "clouderp.py --addons-path=/usr/local/lib/python2.7/dist-packages/gce/addons -d mycompany &"')

def test_deb(o):
    with docker('clouderp-%s-debian-nightly-tests' % version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.deb'
        wheezy.system("service postgresql start")
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('/usr/bin/dpkg -i /opt/release/%s' % wheezy.release)
        wheezy.system('/usr/bin/apt-get install -f -y')
        wheezy.system('su clouderp -s /bin/bash -c "clouderp.py -c /etc/clouderp/gce-server.conf -d mycompany -i base --stop-after-init"')
        wheezy.system('su clouderp -s /bin/bash -c "clouderp.py -c /etc/clouderp/gce-server.conf -d mycompany &"')

def test_rpm(o):
    with docker('clouderp-%s-centos-nightly-tests' % version, o.build_dir, o.pub) as centos7:
        ...
        centos7.system('su clouderp -s /bin/bash -c "gce-server -c /etc/clouderp/gce-server.conf -d mycompany -i base --stop-after-init"')
        centos7.system('su clouderp -s /bin/bash -c "gce-server -c /etc/clouderp/gce-server.conf -d mycompany &"')
...
def options():
    ...
    # Windows VM
    op.add_option("", "--vm-winxp-image", default='/home/clouderp/vm/winxp27/winxp27.vdi', help="%default")
    op.add_option("", "--vm-winxp-ssh-key", default='/home/clouderp/vm/winxp27/id_rsa', help="%default")
    ...
    o.work = join(o.build_dir, 'gce-%s' % o.version_full)
    o.work_addons = join(o.work, 'gce', 'addons')
