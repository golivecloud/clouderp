...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\clouderp Server ${VERSION}"
...
OutFile "gce-allinone-setup-${VERSION}.exe"
...
InstallDir "$PROGRAMFILES\clouderp ${VERSION}"
...
#VIAddVersionKey "LegalTrademark" "gce is a trademark of ${PUBLISHER}"
...
!define MUI_ICON "${PIXMAPS_PATH}\gce-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\gce-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\gce-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\gce-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.golive.pt/page/contactus"
...
; English
LangString DESC_gce_Server ${LANG_ENGLISH} "Install the clouderp Server with all the clouderp standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by clouderp."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact clouderp for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the clouderp Server"
...
LangString TITLE_gce_Server ${LANG_ENGLISH} "clouderp Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start clouderp"

; French
LangString DESC_gce_Server ${LANG_FRENCH} "Installation du Serveur clouderp avec tous les modules clouderp standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par clouderp."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez clouderp pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur clouderp"
...
LangString TITLE_gce_Server ${LANG_FRENCH} "Serveur clouderp"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer clouderp"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_gce_Server) Sectiongce_Server
    SectionIn 1 2
...
# If there is a previous install of the gce Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "addons_path" "$INSTDIR\server\gce\addons"
    WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\gce-server.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    nsExec::Exec '"$INSTDIR\server\gce-server.exe" --stop-after-init --logfile "$INSTDIR\server\gce-server.log" -s'
    nsExec::Exec '"$INSTDIR\service\win32_service.exe" -auto -install'

    # TODO: don't hardcode the service name
    nsExec::Exec "net stop clouderp-server-9.0"
    sleep 2

    nsExec::Exec "net start clouderp-server-9.0"
    sleep 2
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\${POSTGRESQL_EXE_FILENAME}" \
        ...
        --servicename "PostgreSQL_For_clouderp" \
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@golive.pt"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.golive.pt"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@golive.pt"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectiongce_Server} $(DESC_gce_Server)
...
Section "Uninstall"
    ...
    nsExec::Exec "net stop clouderp-server-9.0"
    nsExec::Exec "sc delete clouderp-server-9.0"
    sleep 2
...
Function ShowPostgreSQL
    SectionGetFlags ${Sectiongce_Server} $0
