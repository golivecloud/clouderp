...
execfile(join(dirname(__file__), 'gce', 'release.py'))  # Load release variables
lib_name = 'gce'
...
def py2exe_datafiles():
    ...
    for root, dirnames, filenames in os.walk('gce'):
    ...
    if os.name == 'nt':
        ...
        return {
            'console': [
                {'script': 'clouderp.py'},
                {'script': 'gce-gevent'},
                {'script': 'gce-server', 'icon_resources': [
                    (1, join('setup', 'win32', 'static', 'pixmaps', 'gce-icon.ico'))
                ]},
            ],
            'options': {
                'py2exe': {
...
setup(
    ...
    license=license,
    scripts=['gce-server', 'clouderp.py'],
    packages=find_packages(),
    package_dir={'%s' % lib_name: 'gce'},
