...
Also, adds methods to convert values back to gce models.
"""
...
from gce.tools import ustr, DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT
...
class DateTime(orm.AbstractModel):
    ...
    def from_html(self, cr, uid, model, field, element, context=None):
        ...
        tz_name = context.get('tz') \
            or self.pool['res.users'].read(cr, gce.SUPERUSER_ID, uid, ['tz'], context=context)['tz']
        if tz_name:
            try:
...
class Image(orm.AbstractModel):
    ...
    def load_local_url(self, url):
        ...
        path = gce.modules.get_module_resource(
