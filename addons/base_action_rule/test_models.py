...
class lead_test(osv.Model):
    ...
    customer = gce.fields.Boolean(related='partner_id.customer', readonly=True, store=True)
...
class line_test(osv.Model):
    ...
    name = gce.fields.Char()
    lead_id = gce.fields.Many2one('base.action.rule.lead.test', ondelete='cascade')
    user_id = gce.fields.Many2one('res.users')
