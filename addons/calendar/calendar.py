...
from gce import api
from gce import tools, SUPERUSER_ID
...
class calendar_attendee(osv.Model):
    ...
    def get_ics_file(self, cr, uid, event_obj, context=None):
        ...
        def ics_datetime(idate, allday=False):
            if idate:
                if allday:
                    return gce.fields.Date.from_string(idate)
                else:
                    return gce.fields.Datetime.from_string(idate).replace(tzinfo=pytz.timezone('UTC'))
            return False
        ...
        if event_obj.alarm_ids:
            for alarm in event_obj.alarm_ids:
                ...
                trigger.value = delta
                valarm.add('DESCRIPTION').value = alarm.name or 'clouderp'
...
class calendar_alarm_manager(osv.AbstractModel):
            ...
            for alarm in event.alarm_ids:
                if alarm.type in alarm_type and \
                    one_date - timedelta(minutes=(missing and 0 or alarm.duration_minutes)) < datetime.now() + timedelta(seconds=in_the_next_X_seconds) and \
                        (not after or one_date - timedelta(minutes=alarm.duration_minutes) > gce.fields.Datetime.from_string(after)):
                        alert = {
                            'alarm_id': alarm.id,
                            'event_id': event.id,
    ...
    def get_next_mail(self, cr, uid, context=None):
        now = gce.fields.Datetime.to_string(datetime.now())
...
class calendar_event(osv.Model):
        ...
        if values.get('start_datetime') or values.get('start_date') or values.get('start') \
            ...
            if allday and (values.get('stop_date') or values.get('start_date')):
                ...
                if stop_date and start_date:
                    diff = gce.fields.Date.from_string(stop_date) - gce.fields.Date.from_string(start_date)
                ...
                if stop_datetime and start_datetime:
                    diff = gce.fields.Datetime.from_string(stop_datetime) - gce.fields.Datetime.from_string(start_datetime)
            if diff:
    ...
    def onchange_allday(self, cr, uid, ids, start=False, end=False, starttime=False, endtime=False, startdatetime=False, enddatetime=False, checkallday=False, context=None):
        ...
        if checkallday:  # from datetime to date
            ...
            if starttime:
                start = gce.fields.Datetime.from_string(starttime)
    ...
    def get_search_fields(self, browse_event, order_fields, r_date=None):
        ...
        for ord in order_fields:
            if ord == 'id' and r_date:
                ...
                if type(browse_event[ord]) is gce.osv.orm.browse_record:
    ...
    def _parse_rrule(self, rule, data, date_start):
        ...
        #repeat yearly but for gce it's monthly, take same information as monthly but interval is 12 times
        if r._bymonth:
    ...
    def get_interval(self, cr, uid, ids, date, interval, tz=None, context=None):
        ...
        date = gce.fields.Datetime.from_string(date)
...
class ir_http(osv.AbstractModel):
    ...
    def _auth_method_calendar(self):
        ...
        registry = gce.modules.registry.RegistryManager.get(db)
        attendee_pool = registry.get('calendar.attendee')
        error_message = False
        with registry.cursor() as cr:
            attendee_id = attendee_pool.search(cr, gce.SUPERUSER_ID, [('access_token', '=', token)])
            if not attendee_id:
                error_message = """Invalid Invitation Token."""
            elif request.session.uid and request.session.login != 'anonymous':
                 # if valid session but user is not match
                attendee = attendee_pool.browse(cr, gce.SUPERUSER_ID, attendee_id[0])
                user = registry.get('res.users').browse(cr, gce.SUPERUSER_ID, request.session.uid)
