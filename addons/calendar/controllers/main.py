import json
import gce
import gce.http as http
from gce.http import request
import gce.addons.web.controllers.main as webmain
import json
...
class meeting_invitation(http.Controller):
    ...
    def accept(self, db, token, action, id, **kwargs):
        ...
        with registry.cursor() as cr:
            attendee_id = attendee_pool.search(cr, gce.SUPERUSER_ID, [('access_token', '=', token), ('state', '!=', 'accepted')])
            if attendee_id:
                attendee_pool.do_accept(cr, gce.SUPERUSER_ID, attendee_id)
    ...
    def declined(self, db, token, action, id):
        ...
        attendee_pool = registry.get('calendar.attendee')
        with registry.cursor() as cr:
            attendee_id = attendee_pool.search(cr, gce.SUPERUSER_ID, [('access_token', '=', token), ('state', '!=', 'declined')])
            if attendee_id:
                attendee_pool.do_decline(cr, gce.SUPERUSER_ID, attendee_id)
        return self.view(db, token, action, id, view='form')
    ...
    def view(self, db, token, action, id, view='calendar'):
        ...
        with registry.cursor() as cr:
            attendee = attendee_pool.search_read(cr, gce.SUPERUSER_ID, [('access_token', '=', token)], [])

            if attendee and attendee[0] and attendee[0].get('partner_id'):
                partner_id = int(attendee[0].get('partner_id')[0])
                tz = partner_pool.read(cr, gce.SUPERUSER_ID, partner_id, ['tz'])['tz']
            else:
                tz = False

            attendee_data = meeting_pool.get_attendee(cr, gce.SUPERUSER_ID, id, dict(tz=tz))
        ...
        values = dict(
            init = """
                clouderp.define('calendar.invitation_page', function (require) {
