...
from gce.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            def notify():
                with gce.sql_db.db_connect('postgres').cursor() as cr:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, options=None, timeout=TIMEOUT):
        ...
        if not gce.evented:
            ...
            current.setName("gce.longpolling.request.%s" % current.ident)

        registry = gce.registry(dbname)

        # immediatly returns if past notifications exist
        with registry.cursor() as cr:
            notifications = registry['bus.bus'].poll(cr, gce.SUPERUSER_ID, channels, last, options)
        ...
        if not notifications:
            ...
            try:
                ...
                with registry.cursor() as cr:
                    notifications = registry['bus.bus'].poll(cr, gce.SUPERUSER_ID, channels, last, options, force_status=True)
            except Exception:
                # timeout
                pass
