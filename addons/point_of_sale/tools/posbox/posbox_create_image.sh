...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/clouderp"
mkdir "${CLONE_DIR}"
git clone -b 8.0 --no-checkout --depth 1 https://github.com/golivecloud/clouderp.git "${CLONE_DIR}"
cd "${CLONE_DIR}"
...
gce/
clouderp.py" | tee --append .git/info/sparse-checkout > /dev/null
