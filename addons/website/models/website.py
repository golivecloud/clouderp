...
from gce.osv import orm, osv, fields
from gce.tools import html_escape as escape, ustr, image_resize_and_sharpen, image_save_for_web
from gce.tools.safe_eval import safe_eval
...
def is_multilang_url(local_url, langs=None):
    ...
    try:
        ...
        # Force to check method to POST. clouderp uses methods : ['POST'] and ['GET', 'POST']
...
class website(osv.osv):
    ...
    _defaults = {
        'user_id': lambda self,cr,uid,c: self.pool['ir.model.data'].xmlid_to_res_id(cr, gce.SUPERUSER_ID, 'base.public_user'),
        'company_id': lambda self,cr,uid,c: self.pool['ir.model.data'].xmlid_to_res_id(cr, gce.SUPERUSER_ID,'base.main_company'),
        'compress_html': False,
        'cdn_activated': False,
        'cdn_url': '',
    ...
    def new_page(self, cr, uid, name, template='website.default_page', ispage=True, context=None):
        ...
        while view.search(cr, gce.SUPERUSER_ID, [('key', '=', page_xmlid), '|'] + dom, context=dict(context or {}, active_test=False)):
            inc += 1
    ...
    @gce.tools.ormcache('domain_name')
...
class res_company(osv.osv):
    ...
    def google_map_img(self, cr, uid, ids, zoom=8, width=298, height=298, context=None):
        partner = self.browse(cr, gce.SUPERUSER_ID, ids[0], context=context).partner_id
