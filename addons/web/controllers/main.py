...
from gce.addons.base.ir.ir_qweb import AssetsBundle, QWebTemplateNotFound
from gce.modules import get_resource_path
from gce.tools import topological_sort
from gce.tools.translate import _
from gce.tools import ustr
from gce import http
from gce.http import request, serialize_exception as _serialize_exception
from gce.exceptions import AccessError
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('gce.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, gce.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = gce.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("gce.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, gce has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "gce-web" in x.auto_comments:
...
def db_info():
    version_info = gce.service.common.exp_version()
    return {
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = gce.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except gce.exceptions.AccessDenied:
...
class WebClient(http.Controller):
    ...
    def translations(self, mods=None, lang=None):
        ...
        messages = ir_translation.search_read(request.cr, uid, [('module','in',mods),('lang','=',lang),
                                               ('comments','like','gce-web'),('value','!=',False),
                                               ('value','!=','')],
    ...
    def version_info(self):
        return gce.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-gce-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = gce.tools.config['admin_passwd'] == 'admin'
        d['list_db'] = gce.tools.config['list_db']
        d['langs'] = gce.service.db.exp_list_lang()
        d['countries'] = gce.service.db.exp_list_countries()
        # databases list
        d['databases'] = []
        try:
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            gce.service.db.check_super(master_pwd)
            ...
            dump_stream = gce.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=gce.SUPERUSER_ID))
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas', filename_field='datas_fname', unique=None, filename=None, mimetype=None, download=None, width=0, height=0):
        ...
        if content and (width or height):
            ...
            if height > 500:
                height = 500
            content = gce.tools.image_resize_image(base64_source=content, size=(width or None, height or None), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = gce.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.iteritems(),
            key=lambda field: gce.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from gce's export data to whatever the
