            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: clouderp.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = clouderp.__DEBUG__.get_dependencies( job.name );
                ...
                var missing = clouderp.__DEBUG__.get_missing_jobs();
                var failed = clouderp.__DEBUG__.get_failed_jobs();
                ...
                if (clouderp.debug && !_.isEmpty(debug_jobs)) log.push('\nDebug:                  ', debug_jobs);

                if (clouderp.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            clouderp.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
