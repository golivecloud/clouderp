...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function(parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.clouderp._modules && window.clouderp._modules.slice()) || [];
        this.server = null;
    ...
    user_has_group: function(group) {
        ...
        if (!def) {
            var Model = window.gce.web.Model;
    ...
    load_currencies: function() {
        this.currencies = {};
        var self = this;
        return new gce.web.Model("res.currency").query(["symbol", "position", "decimal_places"]).all()
                .then(function(value) {
    ...
    on_modules_loaded: function() {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            gce[mod] = {};
            // init module mod
            var fct = gce._gce[mod];
            if(typeof(fct) === "function") {
                gce._gce[mod] = {};
                for (var k in fct) {
                    gce._gce[mod][k] = fct[k];
                }
                fct(gce, gce._gce[mod]);
            }
        ...
        return self.check_session_id().then(function() {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-gce-Session-Id"] = self.session_id || '';
                }
...
clouderp.define('web.config', function (require) {
"use strict";
