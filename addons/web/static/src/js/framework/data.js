clouderp.define('web.data', function (require) {
"use strict";
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<gce.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of gce records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the gce model this dataset will manage
     */
    ...
    get_id_index: function(id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // gce ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // gce ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, gce domain
     ...
     * an SQL-like form usable by gce's ORM.
     *
     ...
     * clouderp's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
