clouderp.define('web.UserMenu', function (require) {
"use strict";
...
var UserMenu = Widget.extend({
    ...
    on_menu_documentation: function () {
        window.open('https://www.golive.pt/documentation/user', '_blank');
    },
    on_menu_support: function () {
        window.open('https://www.golive.pt/buy', '_blank');
    },
    on_menu_about: function () {
        var self = this;
        if (clouderp.db_info) {
            menu_help_about(clouderp.db_info);
        } else {
    ...
    on_menu_account: function() {
        this.getParent().clear_uncommitted_changes().then(function() {
            ...
            P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                ...
                framework.redirect('https://accounts.golive.pt/oauth2/auth?'+$.param(params));
            }).fail(function(result, ev){
                ev.preventDefault();
                framework.redirect('https://accounts.golive.pt/account');
            });
        });
    },
