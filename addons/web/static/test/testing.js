...
clouderp.testing.start_services = function () {
    var factories = clouderp.__DEBUG__.factories;
  ...
  return clouderp.process_jobs(jobs, services);
};

clouderp.testing.MockRPC = function (session) {
    this.responses = {};
};

clouderp.testing.MockRPC.prototype.clear = function () {
    this.responses = {};
};

clouderp.testing.MockRPC.prototype.interceptRPC = function (session) {
    session.rpc = this.rpc.bind(this);
};

clouderp.testing.MockRPC.prototype.add = function (spec, handler, no_override) {
...
clouderp.testing.MockRPC.prototype.rpc =  function (url, rparams, options) {
    if (_.isString(url)) {
        url = {url: url};
    }
    var fn, params;
...
clouderp.testing.noop = function () {};

clouderp.define_section = function (name, section_deps) {
    ...
    function test () { 
        ...
        QUnit.test(name, function (assert) {
            var services = clouderp.testing.start_services();
            var deps = _.map(section_deps.concat(dep_names), function (name) { return services[name]; });
            services.qweb.add_template(clouderp.testing.templates);
            mock.clear();
    ...
    var mock = new clouderp.testing.MockRPC();
...
(new QWeb2.Engine()).load_xml("/web/webclient/qweb", function (err, xDoc) {
    clouderp.testing.templates = xDoc;
    QUnit.start();
});
