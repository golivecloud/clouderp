...
example in gce is dashboards), queries should be restricted to a
...
gce Web's :js:class:`~gce.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <gce.web.Widget.$el>`,
...
Known under many names, these objects are essential to and (in gce
...
* All interactive components (components displaying information to the
  ...
  :class:`~gce.web.Widget` and correctly implement and use its API
  and lifecycle.
