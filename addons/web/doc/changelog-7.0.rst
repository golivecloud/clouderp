API changes from gce Web 6.1 to 7.0
=======================================

Supported browsers
------------------

The gce Web Client supports the following web browsers:
...
matches the API in gce Web's Python side and in OpenObject addons
...
* Actual arbitrary RPC calls can just be remapped on a
  :js:class:`~gce.web.Model` instance:
  ...
  can be replaced by calls to :js:func:`gce.web.Model.call`:
...
* Shorcut methods (``name_get``, ``name_search``, ``unlink``,
  ...
  :js:func:`gce.web.Model.call`, using the server's original
  signature. On the other hand, the non-shortcut equivalents can now
  use keyword arguments (see :js:func:`~gce.web.Model.call`'s
...
* ``read_slice``, which allowed a single round-trip to perform a
  ...
  :js:class:`~gce.web.Query` objects (see:
  :js:func:`~gce.web.Model.query`) for clearer and simpler
  code. ``read_index`` should be replaced by a
  :js:class:`~gce.web.Query` as well, combining
  :js:func:`~gce.web.Query.offset` and
  :js:func:`~gce.web.Query.first`.
...
Renaming
    ...
    database, maybe lazily). gce Web's dataset behaves very
...
API simplification
    ...
    The shortcuts really add little value, and gce Web 6.2 embeds
...
:js:class:`~gce.web.Model`, gce Web 7.0 removes
``DataGroup`` and its subtypes as public objects in favor of a single method on
:js:class:`~gce.web.Query`:
:js:func:`~gce.web.Query.group_by`.
