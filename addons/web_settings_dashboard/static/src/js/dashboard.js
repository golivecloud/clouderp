clouderp.define('web_settings_dashboard', function (require) {
"use strict";
...
var DashboardApps = Widget.extend({
    ...
    start: function() {
        this._super.apply(this, arguments);
        if (clouderp.db_info && clouderp.db_info.server_version_info[5] === 'c') {
    ...
    confirm_upgrade: function() {
        framework.redirect("https://www.golive.pt/clouderp-enterprise/upgrade?num_users=" + (this.data.enterprise_users || 1));
    },
});
...
var DashboardShare = Widget.extend({
    ...
    init: function(parent, data){
        this.data = data;
        this.parent = parent;
        this.share_url = 'https://www.golive.pt';
        this.share_text = encodeURIComponent("I am using #clouderp - Awesome open source business apps.");
    },
    ...
    share_linkedin: function(){
        var popup_url = _.str.sprintf('http://www.linkedin.com/shareArticle?mini=true&url=%s&title=I am using clouderp&summary=%s&source=www.golive.pt', encodeURIComponent(this.share_url), this.share_text);
        this.sharer(popup_url);
    },
