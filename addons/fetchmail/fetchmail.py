...
from gce import tools, api, SUPERUSER_ID
...
class fetchmail_server(osv.osv):
    ...
    _defaults = {
        ...
        'priority': 5,
        'attach': True,
        'script': '/mail/static/scripts/gce_mailgate.py',
    }
...
gce_mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s

Example configuration for the postfix mta running locally:

/etc/postfix/virtual_aliases:
@youdomain gce_mailgate@localhost

/etc/aliases:
gce_mailgate: "|/path/to/gce-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
