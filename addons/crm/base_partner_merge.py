...
class MergePartnerAutomatic(osv.TransientModel):
    ...
    def _update_foreign_keys(self, cr, uid, src_partners, dst_partner, context=None):
        ...
        for table, column in cr.fetchall():
            ...
            if len(columns) <= 1:
                ...
                try:
                    with mute_logger('gce.sql_db'), cr.savepoint():
    ...
    def _update_reference_fields(self, cr, uid, src_partners, dst_partner, context=None):
        ...
        def update_records(model, src, field_model='model', field_id='res_id', context=None):
            ...
            ids = proxy.search(cr, gce.SUPERUSER_ID, domain, context=context)
            try:
                with mute_logger('gce.sql_db'), cr.savepoint():
                    return proxy.write(cr, gce.SUPERUSER_ID, ids, {field_id: dst_partner.id}, context=context)
            except psycopg2.Error:
                # updating fails, most likely due to a violated unique constraint
                # keeping record with nonexistent partner_id is useless, better delete it
                return proxy.unlink(cr, gce.SUPERUSER_ID, ids, context=context)
        ...
        record_ids = proxy.search(cr, gce.SUPERUSER_ID, domain, context=context)

        for record in proxy.browse(cr, gce.SUPERUSER_ID, record_ids, context=context):
            try:
            ...
            for partner in src_partners:
                ...
                model_ids = proxy_model.search(cr, gce.SUPERUSER_ID, domain, context=context)
                values = {
                    record.name: 'res.partner,%d' % dst_partner.id,
                }
                proxy_model.write(cr, gce.SUPERUSER_ID, model_ids, values, context=context)
    ...
    @mute_logger('gce.osv.expression', 'gce.models')
    def _merge(self, cr, uid, partner_ids, dst_partner=None, context=None):
        ...
        if gce.SUPERUSER_ID != uid and len(set(partner.email for partner in proxy.browse(cr, uid, partner_ids, context=context))) > 1:
        ...
        if gce.SUPERUSER_ID != uid and self._model_is_installed(cr, uid, 'account.move.line', context=context) and \
                self.pool.get('account.move.line').search(cr, gce.SUPERUSER_ID, [('partner_id', 'in', [partner.id for partner in src_partners])], context=context):
