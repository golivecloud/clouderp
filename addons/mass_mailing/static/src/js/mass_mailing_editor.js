clouderp.define('mass_mailing.editor', function (require) {
"use strict";
...
var _set_value = window.top.clouderp[callback+"_updown"];
var clouderp_top = window.top.clouderp;
window.top.clouderp[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window) {
        delete clouderp_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && web_editor.editor_bar) {
        ...
        if (value.indexOf('on_change_model_and_list') !== -1) {
            clouderp_top[callback+"_downup"](_val);
        }
    }
};
