...
import gce
from gce import tools, _
from gce.addons.web import http
from gce.addons.web.controllers.main import binary_content
from gce.addons.web.http import request
from gce.addons.website.models.website import slug
...
class WebsiteForum(http.Controller):
    ...
    def user_avatar(self, user_id=0, **post):
        status, headers, content = binary_content(model='res.users', id=user_id, field='image_medium', default_mimetype='image/png', env=request.env(user=gce.SUPERUSER_ID))

        if not content:
            img_path = gce.modules.get_module_resource('web', 'static/src/img', 'placeholder.png')
