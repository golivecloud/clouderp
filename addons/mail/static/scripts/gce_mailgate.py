...
class DefaultConfig(object):
    ...
    """
    gce_DEFAULT_USER_ID = 1
    gce_DEFAULT_PASSWORD = 'admin'
    gce_HOSTNAME = 'localhost'
    gce_PORT = 8069
    gce_DEFAULT_DATABASE = 'gce'
...
class RPCProxy(object):
    def __init__(self, uid, passwd,
                 host=config.gce_HOSTNAME,
                 port=config.gce_PORT,
                 path='object',
                 dbname=config.gce_DEFAULT_DATABASE):
...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the clouderp server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="clouderp user id to connect with",
                      default=config.gce_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="clouderp user password",
                      default=config.gce_DEFAULT_PASSWORD)
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="clouderp database name (default: %default)",
                      default=config.gce_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="clouderp Server hostname",
                      default=config.gce_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="clouderp Server XML-RPC port number",
                      default=config.gce_PORT)
...
def main():
    """
    Receive the email via the stdin and send it to the gce Server
    """
    ...
    try:
        ...
        subject = '[clouderp]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to clouderp Server, sending error notification to %s\n" % config.MAIL_ADMINS)
