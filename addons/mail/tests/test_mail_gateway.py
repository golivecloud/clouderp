...
To: {to}
cc: {cc}
Received: by mail1.gce.com (Postfix, from userid 10002)
...
Received: from mail-ie0-f173.google.com (mail-ie0-f173.google.com [209.85.223.173])
    ...
    for <raoul@gce.fr>; Fri, 23 Aug 2013 13:17:55 +0200 (CEST)
...
class TestMailgateway(TestMail):
    ...
    def setUp(self):
        ...
        self.fake_email = self.env['mail.message'].create({
            ...
            'message_id': '<123456-gce-%s-mail.channel@%s>' % (self.group_public.id, socket.gethostname()),
        })

    @mute_logger('gce.addons.mail.models.mail_thread')
    ...
    @mute_logger('gce.addons.mail.models.mail_thread', 'gce.models')
    ...
    @mute_logger('gce.addons.mail.models.mail_thread', 'gce.models', 'gce.addons.mail.models.mail_mail')
    ...
    def test_message_process_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since gce saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process,
                          MAIL_TEMPLATE, email_from='valid.lelitre@agrolait.com',
                          to='noone@example.com', subject='spam',
                          extra='In-Reply-To: <12321321-gce-%d-mail.channel@%s>' % (self.group_public.id, socket.gethostname()),
        ...
        self.assertRaises(ValueError,
                          ...
                          to='noone@example.com>', subject='spam',
                          extra='In-Reply-To: <12321321-gce-%d-mail.channel@neighbor.com>' % frog_groups.id)
    ...
    def test_private_discussion(self):
        ...
        # Test: message-id
        self.assertIn('gce-private', msg.message_id, 'message_post: private discussion: message-id should contain the private keyword')
