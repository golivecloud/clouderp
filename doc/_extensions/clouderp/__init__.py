...
def setup(app):
    if getattr(app.config, 'html_translator_class', None):
        app.warn("Overriding the explicitly set  html_translator_class setting",
                 location="clouderp extension")
    app.config.html_translator_class = 'clouderp.translator.BootstrapTranslator'

    switcher.setup(app)
    app.add_config_value('clouderp_cover_default', None, 'env')
    app.add_config_value('clouderp_cover_external', {}, 'env')
    app.add_config_value('clouderp_cover_default_external', lambda conf: conf.clouderp_cover_default, 'env')
...
def update_meta(app, pagename, templatename, context, doctree):
    ...
    meta.setdefault('banner', app.config.clouderp_cover_default)
