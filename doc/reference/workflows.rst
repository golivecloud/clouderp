...
In clouderp, a workflow is a technical artefact to manage a set of "things to
...
- Activities define work that should be done within the clouderp server, such
...
All in all, clouderp's workflow system provides:
...
A company using clouderp may want to add discount support to orders, where sales
...
clouderp knows where to start the workflow traversal after it is instanciated.
...
for a record, clouderp simply processes all of them, and evaluate all their
...
It is important for clouderp to know when a workflow instance is completed. A
...
giving a signal name in the attribute ``signal_send``. clouderp processes those
...
After an activity has been processed, clouderp evaluates its transition to reach
the next activity in the flow.

However if an activity has more than one transition, clouderp must decide which
...
``XOR`` (default)
    By default, clouderp will use the first transition (in ``sequence`` order)
...
``OR``
    ...
    become valid later.
``AND``
    In ``AND`` mode, clouderp will wait until *all* transitions are satisfied, and
...
signal or trigger is defined), the condition is evaluated by clouderp, and if
...
defined (in addition to the clouderp ``safe_eval`` environment):
