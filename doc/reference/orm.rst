...
.. versionadded:: 8.0
    ...
    This page documents the New API added in clouderp 8.0 which should be the
...
Accessing a relational field (:class:`~gce.fields.Many2one`,
:class:`~gce.fields.One2many`, :class:`~gce.fields.Many2many`)
...
.. danger::
    ...
    (to the same value), use :meth:`~gce.models.Model.write`::
...
:meth:`~gce.models.Model.filtered`
...
The :class:`~gce.api.Environment` stores various contextual data used by
...
using :attr:`~gce.models.Model.env` and gives access to the current user
(:attr:`~gce.api.Environment.user`), the cursor
(:attr:`~gce.api.Environment.cr`) or the context
(:attr:`~gce.api.Environment.context`)::
...
:meth:`~gce.models.Model.with_context`
...
:meth:`~gce.models.Model.with_env`
...
:meth:`~gce.models.Model.search`
   ...
   .. tip:: to just check if any record matches a domain, or count the number
             ...
             :meth:`~gce.models.Model.search_count`
:meth:`~gce.models.Model.create`
...
:meth:`~gce.models.Model.ensure_one`
...
* computed fields are automatically checked and recomputed, they do not need
  an ``onchange``
* for non-computed fields, the :func:`~gce.api.onchange` decorator is used
...
The :attr:`~gce.api.Environment.cr` attribute on environments is the
...
Because models use the same cursor and the :class:`~gce.api.Environment`
...
:meth:`~gce.api.Environment.invalidate_all` method of the
:class:`~gce.api.Environment` object.
...
clouderp is currently transitioning from an older (less regular) API, it can be
...
* values of the :class:`~gce.api.Environment` (cursor, user id and
  context) are passed explicitly to methods instead
* record data (:attr:`~gce.models.Model.ids`) are passed explicitly to
...
.. currentmodule:: gce.models

.. autoclass:: gce.models.Model
    ...
    .. attribute:: _table
        ...
        :attr:`~gce.models.Model._auto`, automatically generated by
        default.
    ...
    .. attribute:: _inherits
        ...
        the fields of the :attr:`~gce.models.Model._inherits`-ed model but
        ...
        .. warning::
            ...
            :attr:`~gce.models.Model._inherits`-ed
    ...
    .. attribute:: id
        ...
        Identifier :class:`field <gce.fields.Field>`
    ...
    .. attribute:: sequence
        ...
        :type: :class:`~gce.fields.Integer`

    .. attribute:: state

        lifecycle stages of the object, used by the ``states`` attribute on
        :class:`fields <gce.fields.Field>`

        :type: :class:`~gce.fields.Selection`
    ...
    .. attribute:: parent_id
        ...
        :type: :class:`~gce.fields.Many2one`
...
.. autodoc documents descriptors as attributes, even for the *definition* of
   descriptors. As a result automodule:: gce.fields lists all the field
...
.. autoclass:: gce.fields.Field

.. autoclass:: gce.fields.Char
    :show-inheritance:

.. autoclass:: gce.fields.Boolean
    :show-inheritance:

.. autoclass:: gce.fields.Integer
    :show-inheritance:

.. autoclass:: gce.fields.Float
    :show-inheritance:

.. autoclass:: gce.fields.Text
    :show-inheritance:

.. autoclass:: gce.fields.Selection
    :show-inheritance:

.. autoclass:: gce.fields.Html
    :show-inheritance:

.. autoclass:: gce.fields.Date
    :show-inheritance:
    :members: today, context_today, from_string, to_string

.. autoclass:: gce.fields.Datetime
    :show-inheritance:
...
.. autoclass:: gce.fields.Many2one
    :show-inheritance:

.. autoclass:: gce.fields.One2many
    :show-inheritance:

.. autoclass:: gce.fields.Many2many
    :show-inheritance:

.. autoclass:: gce.fields.Reference
    :show-inheritance:
...
clouderp provides three different mechanisms to extend models in a modular way:
...
When using the :attr:`~gce.models.Model._inherit` and
:attr:`~gce.models.Model._name` attributes together, clouderp creates a new
model using the existing one (provided via
:attr:`~gce.models.Model._inherit`) as a base. The new model gets all the
fields, methods and meta-information (defaults & al) from its base.

.. literalinclude:: ../../gce/addons/test_documentation_examples/inheritance.py
    :language: python
    :lines: 5-

and using them:

.. literalinclude:: ../../gce/addons/test_documentation_examples/tests/test_inheritance.py
    :language: python
    :lines: 8,12,9,19
...
When using :attr:`~gce.models.Model._inherit` but leaving out
:attr:`~gce.models.Model._name`, the new model replaces the existing one,
...
.. literalinclude:: ../../gce/addons/test_documentation_examples/extension.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../gce/addons/test_documentation_examples/tests/test_extension.py
    :language: python
    :lines: 8,13
...
at runtime) but less power: using the :attr:`~gce.models.Model._inherits`
...
:class:`~gce.fields.Reference` fields automatically set up on the parent
model:

.. literalinclude:: ../../gce/addons/test_documentation_examples/delegation.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../gce/addons/test_documentation_examples/tests/test_delegation.py
    :language: python
    :lines: 9-12,21,26
...
``field_name`` (``str``)
    ...
    a :class:`~gce.fields.Many2one` using dot-notation e.g. ``'street'``
...
``operator`` (``str``)
    ...
    ``child_of``
        ...
        :attr:`~gce.models.Model._parent_name`).
...
* :meth:`~gce.models.Model.search` returns a recordset, no point in e.g.
  browsing its result
* ``fields.related`` and ``fields.function`` are replaced by using a normal
  field type with either a ``related=`` or a ``compute=`` parameter
* :func:`~gce.api.depends` on ``compute=`` methods **must be complete**,
...
* the decorators :func:`~gce.api.model` and :func:`~gce.api.multi` are
  for bridging *when calling from the old API context*, for internal or pure
  new-api (e.g. compute) they are useless
* remove :attr:`~gce.models.Model._default`, replace by ``default=``
...
* double check that all fields and methods have different names, there is no
  warning in case of collision (because Python handles it before clouderp sees
  anything)
* the normal new-api import is ``from gce import fields, models``. If
  compatibility decorators are necessary, use ``from gce import api,
  fields, models``
* avoid the :func:`~gce.api.one` decorator, it probably does not do what
  you expect
* remove explicit definition of :attr:`~gce.models.Model.create_uid`,
  :attr:`~gce.models.Model.create_date`,
  :attr:`~gce.models.Model.write_uid` and
  :attr:`~gce.models.Model.write_date` fields: they are now created as
...
* when straight conversion is impossible (semantics can not be bridged) or the
  ...
  implementations for the same method name using :func:`~gce.api.v7` and
  :func:`~gce.api.v8`. The method should first be defined using the
  old-API style and decorated with :func:`~gce.api.v7`, it should then be
  re-defined using the exact same name but the new-API style and decorated
  with :func:`~gce.api.v8`. Calls from an old-API context will be
...
* uses of :attr:`~gce.models.Model._columns` or
  :attr:`~gce.models.Model._all_columns` should be replaced by
  :attr:`~gce.models.Model._fields`, which provides access to instances of
  new-style :class:`gce.fields.Field` instances (rather than old-style
  :class:`gce.osv.fields._column`).

  Non-stored computed fields created using the new API style are *not*
  available in :attr:`~gce.models.Model._columns` and can only be
  inspected through :attr:`~gce.models.Model._fields`
* reassigning ``self`` in a method is probably unnecessary and may break
  translation introspection
* :class:`~gce.api.Environment` objects rely on some threadlocal state,
  which has to be set up before using them. It is necessary to do so using the
  :meth:`gce.api.Environment.manage` context manager when trying to use
  the new API in contexts where it hasn't been set up yet, such as new threads
  or a Python interactive environment::

    >>> from gce import api, modules
...
:class:`~gce.api.Environment` (for :attr:`~gce.api.Environment.cr`,
:attr:`~gce.api.Environment.user` and
:attr:`~gce.api.Environment.context`) or the current recordset (for ``id``
and ``ids``).
...
* disabling it entirely, by decorating a method with
  :func:`~gce.api.noguess` there will be no bridging and methods will be
...
* defining the bridge explicitly, this is mostly for methods which are matched
  ...
  (e.g. :func:`~gce.api.cr_uid_context`) which also passes the current
  context *by keyword*.
* dual implementations using :func:`~gce.api.v7` and
  :func:`~gce.api.v8` will be ignored as they provide their own "bridging"
