...
automatically loaded by clouderp when the corresponding language is installed (via
...
clouderp automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~gce.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~gce.models.Model._constraints` and
  :attr:`~gce.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, clouderp cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`gce._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`gce.web._t`:
