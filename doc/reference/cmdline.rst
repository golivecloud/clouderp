...
===============================
Command-line interface: clouderp.py
===============================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.gce_serverrc` by default, and can be overridden using
    :option:`-c`)
...
.. option:: --db-filter <filter>
    ...
    - ``%d`` is replaced by the subdomain the request is made on, with the
      exception of ``www`` (so domain ``golive.pt`` and ``www.golive.pt`` both
      match the database ``clouderp``)
...
By default, clouderp displays all logging of level_ ``info`` except for workflow
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``gce.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ clouderp.py --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler gce.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=gce.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=gce.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=gce.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=gce.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``gce`` and ``werkzeug`` loggers (except for ``debug`` which is only
    set on ``gce``).

    clouderp also provides debugging pseudo-levels which apply to different sets
    of loggers:
    ...
    ``debug_rpc``
        sets the ``gce`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``gce`` and HTTP request and response loggers to
        ``debug``
...
.. program:: clouderp.py scaffold

Scaffolding is the automated creation of a skeleton structure to simplify
bootstrapping (of new modules, in the case of clouderp). While not necessary it
avoids the tedium of setting up basic structures and looking up what all
starting requirements are.

Scaffolding is available via the :command:`clouderp.py scaffold` subcommand.
...
The default configuration file is :file:`{$HOME}/.gce_serverrc` which
can be overridden using :option:`--config <clouderp.py -c>`. Specifying
:option:`--save <clouderp.py -s>` will save the current configuration state back
to that file.
