...
``views``
    ...
    :meth:`~gce.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an clouderp action. Can be customized
via two fields:
...
will replace the current content section by the clouderp home page.
...
``model_id``
    clouderp model linked to the action, made available in
...
Creates a new record, from scratch (via :meth:`~gce.models.Model.create`)
or by copying an existing record (via :meth:`~gce.models.Model.copy`)
...
``fields_lines``
    ...
    :class:`~gce.fields.One2many` with the fields:
...
``crud_model_id``
    ...
    ``new_other``
``ref_object``
    :class:`~gce.fields.Reference` to an arbitrary record to copy, used if
...
``wkf_transition_id``
    :class:`~gce.fields.Many2one` to a ``workflow.transition`` to trigger
...
``groups_id``
    ...
    the current report
``paperformat_id``
    :class:`~gce.fields.Many2one` field to the paper format you wish to
