...
clouderp is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in clouderp is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``clouderp``
...
``search``
    ...
    the field is a :class:`~gce.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~gce.fields.Many2one` and
    :class:`~gce.fields.Reference` fields
``type``
...
Because some important structural models of clouderp are complex and involved,
...
.. literalinclude:: ../../gce/addons/base/res/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../gce/addons/base/res/res.country.state.csv
    :header-rows: 1
