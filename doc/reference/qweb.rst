...
QWeb is the primary templating_ engine used by clouderp\ [#othertemplates]_. It
...
:meth:`gce.http.HttpRequest.render`:
...
This automatically creates a :class:`~gce.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~gce.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the clouderp model to use for rendering, can be
...
.. automodule:: gce.addons.base.ir.ir_qweb
...
.. js:class:: QWeb2.Engine
    ...
    gce Web instantiates one for the user in the core module, and 
    ...
    gce Web, Kanban views get their own :js:class:`QWeb2.Engine`
    ...
    .. js:attribute:: QWeb2.Engine.preprocess_node
        ...
        node to template code. In gce Web, this is used to
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. clouderp 9.0 still depends on Jinja_ and Mako_.
