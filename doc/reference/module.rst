...
The manifest file serves to declare a python package as an clouderp module
and to specify module metadata.

It is a file called ``__gce__.py`` and contains a single Python
...
``category`` (``str``, default: ``Uncategorized``)
    classification category within clouderp, rough business domain for the module.
...
``depends`` (``list(str)``)
    clouderp modules which must be loaded before this one, either because this
...
.. _existing categories:
     https://github.com/golivecloud/clouderp/blob/master/gce/addons/base/module/module_data.xml
