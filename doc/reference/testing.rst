...
clouderp provides support for testing modules using unittest.
...
`unittest documentation`_, but clouderp provides a number of utilities and helpers
related to testing clouderp content (modules, mainly):

.. autoclass:: gce.tests.common.TransactionCase
    :members: browse_ref, ref

.. autoclass:: gce.tests.common.SingleTransactionCase
    :members: browse_ref, ref
...
.. autofunction:: gce.tests.common.at_install

.. autofunction:: gce.tests.common.post_install

The most common situation is to use
:class:`~gce.tests.common.TransactionCase` and test a property of a model
in each method::
...
:option:`--test-enable <clouderp.py --test-enable>` was enabled when starting the
clouderp server.

As of clouderp 8, running tests outside of the install/update cycle is not
supported.
