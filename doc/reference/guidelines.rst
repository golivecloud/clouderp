:banner: banners/clouderp_guideline.jpg
...
This page introduces the new clouderp Coding Guidelines. Those aim to improve the quality of the code (e.g. better readability of source) and clouderp Apps. Indeed, proper code eases maintenance, aids debugging, lowers complexity and promotes reliability.
...
.. warning::
    ...
    each line. For more details, see our `pull request guide <https://golive.pt/submit-pr>`_.
...
Don't link data (image, libraries) outside clouderp: do not use an
...
clouderp supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. clouderp
...
#. Imports of ``clouderp``
#. Imports from clouderp modules (rarely, and only if necessary)
...
.. code-block:: python
    ...
    # 2 :  imports of clouderp
    import clouderp
    from clouderp import api, fields, models # alphabetically ordered
    from clouderp.tools.safe_eval import safe_eval as eval
    from clouderp.tools.translate import _
    # 3 :  imports from clouderp modules
    from clouderp.addons.website.models.website import slug
    from clouderp.addons.web.controllers.main import login_redirect
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The clouderp framework is in charge of providing the transactional context for
...
clouderp uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in clouderp, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an clouderp Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an clouderp Transient (wizard) : use ``<related_base_model>.<action>``
...
- clouderp Python Class : use camelcase for code in api v8 (Object-oriented style),
...
.. code-block:: javascript
    ...
    gce.website.if_dom_contains('.jquery_class_selector', function () {
        /*your code here*/
    });
...
- **[CLA]** for signing the clouderp Individual Contributor License
