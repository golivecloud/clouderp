...
``groups_id``
    :class:`~gce.fields.Many2many` field to the groups allowed to view/use
    the current view
``inherit_id``
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects clouderp:
    ...
    ``states``
        ...
        listed states
    ``context``
        merged into the view's context when performing the button's clouderp call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's clouderp call
...
Semantic components tie into and allow interaction with the clouderp
system. Available semantic components are:

``button``
  call into the clouderp system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~gce.fields.Char`,
    :class:`~gce.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`gce.api.onchange` on the model
  ...
  ``mode``
    for :class:`~gce.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~gce.fields.Char` field stores a password and
...
Most :class:`~gce.fields.Many2many` fields, like categories, are better
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~gce.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/golivecloud/clouderp/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``formats``
  ...
  ``read_only_mode``
    ...
    * buttons and links with a ``type`` attribute become perform clouderp-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`clouderp buttons
        <reference/views/list/button>`, most attributes relevant to standard
        clouderp buttons can be used.
      ``open``
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's clouderp model
    ``source`` (required)
      :class:`~gce.fields.Many2one` field of the edge's model pointing to
...
``field``
    ...
    ``widget``
        ...
        standard clouderp 8.0 is a ``selection`` widget for
        :class:`~gce.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~gce.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain``
        an clouderp :ref:`domain <reference/orm/domains>`, will be appended to the
