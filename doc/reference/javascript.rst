...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in clouderp is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  clouderp Web's
...
To display and interact with data, calls to the clouderp server are necessary.
This is performed using :abbr:`RPC <Remote Procedure Call>`.

clouderp Web provides two primary APIs to handle this: a low-level
JSON-RPC based API communicating with the Python section of clouderp
Web (and of your module, if you have a Python part) and a high-level
API above that allowing your code to talk directly to high-level clouderp models.
...
High-level API: calling into clouderp models
-------------------------------------------

Access to clouderp object methods (made available through XML-RPC from the server)
is done via :class:`Model` (exported in ``web.Model``). It maps onto the clouderp server objects via
two primary methods, :func:`~Model.call` and
:func:`~Model.query`.

:func:`~Model.call` is a direct mapping to the corresponding method of
the clouderp server object. Its usage is similar to that of the clouderp Model API,
...
interface to searches (``search`` + ``read`` in clouderp RPC terms). It
returns a :class:`~gce.web.Query` object which is immutable but
allows building new :class:`~gce.web.Query` instances from the
...
serialization methods, :func:`~gce.web.Query.all` and
:func:`~gce.web.Query.first`. These methods will perform a new
...
.. class:: Model(name)
    ...
    .. function:: Model.query(fields)
         ...
         :param Array<String> fields: list of fields to fetch during
                                      the search
         :returns: a :class:`~gce.web.Query` object
                   representing the search to perform

.. class:: gce.web.Query(fields)
    ...
    .. function:: gce.web.Query.all()

        Fetches the result of the current :class:`~gce.web.Query` object's
        search.

        :rtype: Deferred<Array<>>

    .. function:: gce.web.Query.first()

       Fetches the **first** result of the current
       :class:`~gce.web.Query`, or ``null`` if the current
       :class:`~gce.web.Query` does have any result.

       :rtype: Deferred<Object | null>

    .. function:: gce.web.Query.count()

       Fetches the number of records the current
       :class:`~gce.web.Query` would retrieve.

       :rtype: Deferred<Number>

    .. function:: gce.web.Query.group_by(grouping...)
    ...
    **new** :class:`~gce.web.Query` object with the relevant
    ...
    .. function:: gce.web.Query.limit(limit)

       Sets the provided limit on the query. The new limit *replaces*
       the old one.

    .. function:: gce.web.Query.order_by(fields…)
...
clouderp has powerful grouping capacities, but they are kind-of strange
in that they're recursive, and level n+1 relies on data provided
directly by the grouping at level n. As a result, while
:py:meth:`gce.models.Model.read_group` works it's not a very intuitive
API.

clouderp Web eschews direct calls to :py:meth:`~gce.models.Model.read_group`
in favor of calling a method of :class:`~gce.web.Query`, :py:meth:`much
...
The result of a (successful) :func:`~gce.web.Query.group_by` is
an array of :class:`~gce.web.QueryGroup`:

.. class:: gce.web.QueryGroup

    .. function:: gce.web.QueryGroup.get(key)
    ...
    .. function:: gce.web.QueryGroup.query([fields...])

        equivalent to :func:`Model.query` but pre-filtered to
        only include the records within this group. Returns a
        :class:`~gce.web.Query` which can be further manipulated as
        usual.

    .. function:: gce.web.QueryGroup.subgroups()

        returns a deferred to an array of :class:`~gce.web.QueryGroup`
        below this one

Low-level API: RPC calls to Python side
---------------------------------------

While the previous section is great for calling core gce code
...
It has many advantages over the clouderp version 8 system.
...
* files are required to use the module system if they want to interact 
  with clouderp, since the various objects are only available in the module
...
adopt new habits.  For example, the variable gce does not exist
...
simple example::
    ...
    clouderp.define('addon_name.service', function (require) {
...
with the ``clouderp.define`` function.  ``clouderp.define`` takes a name and a
...
* the ``framework/`` folder contains all basic low level modules.  The
  modules here are supposed to be generic.  Some of them are:
    ...
    * ``web.session`` is the former ``gce.session``
...
global variable ``gce``.  In theory, our addons should work without
ever using the variable ``gce``, and the compatibility module can be
disabled safely.
...
Testing in clouderp Web Client
==========================

Javascript Unit Testing
-----------------------

clouderp Web includes means to unit-test both the core code of
clouderp Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with clouderp.

To see what the runner looks like, find (or start) an clouderp server
...
``test`` key of the clouderp manifest, by adding javascript files to it:
...
The next step is to create a test case::
    ...
    gce.testing.section('basic section', function (test) {
        test('my first test', function () {
            ok(false, "this test has run");
        });
    });

All testing helpers and structures live in the ``gce.testing``
module. clouderp tests live in a :func:`~gce.testing.section`,
...
:func:`test <gce.testing.case>`, provided by the
:func:`~gce.testing.section` to the callback, is used to
register a given test case which will be run whenever the test runner
actually does its job. clouderp Web test case use standard `QUnit
...
As noted above, clouderp Web's tests use `qunit assertions`_. They are
...
Getting an clouderp instance
------------------------

The clouderp instance is the base through which most clouderp Web
...
::
    ...
    // src/js/demo.js
    gce.web_tests_demo = function (instance) {
...
e.g. fully initialize :class:`widgets <~gce.Widget>` before
testing them.
...
.. function:: mock(rpc_spec, handler)
    ...
    * If it matches the pattern ``model:method`` (if it contains a
      ...
      call straight to the clouderp server (through XMLRPC) as
      performed via e.g. :func:`gce.web.Model.call`.
    ...
    * Otherwise, if it matches an absolute path (e.g. ``/a/b/c``) it
      ...
      as if returned by the original JSON-RPC handler::
          ...
          test('JSON-RPC', {rpc: 'mock', asserts: 3, templates: true}, function (instance, $s, mock) {
              ...
              instance.webclient = {toggle_bars: gce.testing.noop};
...
.. function:: gce.testing.section(name[, options], body)
    ...
    :param body:
    :type body: Function<:func:`~gce.testing.case`, void>

.. function:: gce.testing.case(name[, options], callback)
...
.. class:: TestOptions
    ...
    :func:`~gce.testing.section` or
    :func:`~gce.testing.case`. Except for
    :attr:`~TestOptions.setup` and
    :attr:`~TestOptions.teardown`, an option on
    :func:`~gce.testing.case` will overwrite the corresponding
    option on :func:`~gce.testing.section` so
    e.g. :attr:`~TestOptions.rpc` can be set for a
    :func:`~gce.testing.section` and then differently set for
    some :func:`~gce.testing.case` of that
    :func:`~gce.testing.section`
...
#. Install a new database with all relevant modules (all modules with
   ...
   .. note::
       ...
       being duplicated, but clouderp doesn't currently break
...
the gce Web test suite.

.. note::

    Note that this runs all the Python tests for the ``web`` module,
    but all the web tests for all of clouderp. This can be surprising.
