...
This guide is about creating modules for clouderp's web client.

To create websites with clouderp, see :doc:`website`; to add business capabilities
or extend existing business systems of clouderp, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed clouderp <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple clouderp module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/golivecloud/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to clouderp's
:option:`addons path <clouderp.py --addons-path>`, create a new database and
...
Files used in the "web" side of an clouderp module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        gce.oepetstore = function(instance, local) {
...
The files in the ``static`` folder, need to be defined within the module in order for them to be loaded correctly. Everything in ``src/xml`` is defined in ``__gce__.py`` while the contents of ``src/css`` and ``src/js`` are defined in ``petstore.xml``, or a similar file.
...
.. warning::
    ...
    "developer mode": log into your clouderp instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    clouderp screen) and select :guilabel:`About clouderp` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_clouderp.png
        :align: center
...
.. todo:: qweb files hooked via __gce__.py, but js and CSS use bundles

clouderp JavaScript Module
======================
...
The clouderp framework uses one such pattern to define modules within web addons,
...
In clouderp web, modules are declared as functions set on the global ``gce``
...
* the first parameter is the current instance of the clouderp web client, it gives
  access to various capabilities defined by the clouderp (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the clouderp web client needs to call them
...
For simplicity and developer-friendliness clouderp web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~gce.web.Class.extend`
method of :class:`gce.web.Class`::
...
The :func:`~gce.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~gce.web.Class.extend` on the parent class, as is done
to subclass :class:`~gce.web.Class`::
...
The clouderp web client bundles jQuery_ for easy DOM manipulation. It is useful
...
clouderp Web makes specific components responsible for sections of a page. In
clouderp web, the base for such components is the :class:`~gce.Widget`
...
It extends :class:`~gce.Widget` and overrides the standard method
:func:`~gce.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~gce.Widget.start`
...
All widgets have a :attr:`~gce.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~gce.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear clouderp user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in clouderp Web, you will need to reload
    the page. There is no need to restart the clouderp server.

The ``HomePage`` widget is used by clouderp Web and managed automatically.
...
``GreetingsWidget``'s :func:`~gce.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~gce.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~gce.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~gce.Widget.start` method will be called during the call
to :func:`~gce.Widget.appendTo`.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear clouderp user!</div>
...
:class:`~gce.Widget`, because we added some classes on them.
...
When overriding the :func:`~gce.Widget.init` method of a widget it is
...
it. This is done via the :func:`~gce.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~gce.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~gce.Widget.init` or :func:`~gce.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~gce.Widget.destroy`.

.. danger::

    when overriding :func:`~gce.Widget.destroy`, ``_super()``
...
As many other environments, clouderp's solution is to use a `template engine`_.
clouderp's template engine is called :ref:`reference/qweb`.
...
* It has special support in clouderp Web's :class:`~gce.Widget`, though it
  can be used outside of clouderp's web client (and it's possible to use
  :class:`~gce.Widget` without relying on QWeb)
...
.. note::
    ...
    like clouderp :ref:`views <reference/views>`.
    ...
    characteristic of clouderp, and losing it was considered unacceptable.
...
However, because :class:`~gce.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~gce.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~gce.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~gce.Widget.template`. If there are
...
When using :class:`~gce.Widget`'s integration it is not possible to
...
before :func:`~gce.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~gce.Widget.init`):
...
But because it's a common operation, :class:`~gce.Widget` provides an
equivalent shortcut through the :func:`~gce.Widget.$` method::
...
:attr:`~gce.Widget.events` is an object (mapping) of an event to the
...
:func:`~gce.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~gce.Widget.on`::
...
:func:`~gce.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~gce.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``gce.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the clouderp web application.
...
* :func:`~gce.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~gce.Widget.set`) and ``change``
* :func:`~gce.Widget.get` retrieves the value of a property.
...
The class system of the clouderp web framework allows direct modification of
existing classes using the :func:`~gce.web.Class.include` method::
...
defined in the call to :func:`~gce.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~gce.Widget.include`.
...
In clouderp, translations files are automatically generated by scanning the source
...
:func:`~gce.web._t` (and also :func:`~gce.web._lt`).
...
:func:`~gce.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the clouderp Server
==================================

Contacting Models
-----------------

Most operations with clouderp involve communicating with *models* implementing
...
communicating with clouderp requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, clouderp web provides
...
The class used to call clouderp models is :class:`gce.Model`. It is
instantiated with the clouderp model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~gce.web.Model.call` can be used to call any (public) method of an
clouderp model. It takes the following positional arguments:
...
:func:`~gce.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if clouderp
...
solution to achieve that is to use :class:`gce.web.CompoundContext`.

:class:`~gce.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~gce.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in clouderp plus the
``new_key`` key that was added when instantiating
:class:`~gce.web.CompoundContext`.

Queries
-------

While :func:`~gce.Model.call` is sufficient for any interaction with clouderp
models, clouderp Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~gce.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~gce.models.Model.search` and
::py:meth:`~gce.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~gce.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`gce.web.Query` which can be further customized before
  being executed
* :class:`~gce.web.Query` represents the query being built. It is
  ...
  :class:`~gce.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~gce.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~gce.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In clouderp, images are generally stored in regular fields encoded as
...
In clouderp, many operations start from an :ref:`action <reference/actions>`:
...
In clouderp Web, the component responsible for handling and reacting to these
...
:func:`~gce.Widget.do_action` is a shortcut of :class:`~gce.Widget`
...
the clouderp web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for clouderp web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~gce.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~gce.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the clouderp web client root.
...
Much of clouderp web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`clouderp views <reference/views>` are implemented through a subclass
of :class:`gce.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main clouderp framework, but
...
:py:class:`~gce.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~gce.web.View.view_id` attribute.

Views are also provided with a :class:`~gce.web.DataSet` instance which
...
:func:`~gce.web.View.do_search`, and updating their
:class:`~gce.web.DataSet` as necessary.
...
:py:class:`~gce.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of clouderp. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in clouderp have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the clouderp server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in clouderp. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the clouderp web
client.
...
user to modify it can be useful, but most fields in clouderp also allow editing.
...
their :attr:`~gce.web.form.FormWidget.field_manager` attribute::
...
:attr:`~gce.web.form.FormWidget` is generally the
:class:`~gce.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~gce.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~gce.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~gce.web.form.FieldManagerMixin.set_values(values)` sets multiple
