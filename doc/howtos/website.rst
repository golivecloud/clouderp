...
.. warning::
    ...
    * This guide assumes :ref:`an installed clouderp <setup/install>`

Creating a basic module
=======================

In clouderp, tasks are performed by creating modules.

Modules customize the behavior of an clouderp installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`clouderp's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./clouderp.py scaffold Academy my-modules
...
* start the clouderp server

  .. code-block:: console

      $ ./clouderp.py --addons-path addons,my-modules
...
display logic. clouderp allows any Python templating system, but provides its
...
``__gce__.py`` manifest, and alter the controller to use our template:
...
Finally restart clouderp and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, clouderp can be restarted :option:`and update modules at
    the same time<clouderp.py -u>`:

    .. code-block:: console

        $ clouderp.py --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8069/academy/academy/ should now result in:

.. image:: website/basic-list.png

Storing data in clouderp
====================

:ref:`clouderp models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~gce.models.Model.search` returns a set of records
...
clouderp bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but clouderp 8 added deeper
...
You're now in clouderp "proper": the administrative interface. For now click on
...
:func:`~gce.http.route` decorator which takes a routing string and a
...
restart clouderp, access http://localhost:8069/academy/Alice/ and
...
Restart clouderp, access http://localhost:8069/academy/2, note how the old value
...
clouderp provides an additional converter called ``model`` which provides records
...
Restart clouderp and upgrade the module, then you can visit each teacher's page.
...
Restart clouderp and update the views, reload the teacher's page and… the field
...
Restart clouderp and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the clouderp administration
--------------------------------------------------------------

The clouderp administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the clouderp backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which clouderp should
   execute or data display. Data display actions are called *window actions*,
   and tell clouderp to display a given *model* according to a set of views…
...
Editing in the clouderp administration
----------------------------------

By default, an clouderp model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) clouderp will automatically create a basic one
...
:class:`the inverse relationship <gce.fields.One2many>` to the *teachers*
model:
...
clouderp provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of clouderp's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~gce.models.Model._inherit`
...
clouderp also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the clouderp system. We can easily make
...
restart clouderp, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <gce.models.Model._inherit>` without giving it a new
      :attr:`~gce.models.Model._name`
