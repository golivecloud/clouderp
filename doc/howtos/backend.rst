...
.. warning::
    ...
    This tutorial requires :ref:`having installed clouderp <setup/install>`

Start/Stop the clouderp server
==========================

clouderp uses a client/server architecture in which clients are web browsers
accessing the clouderp server via RPC.
...
In order to start the server, simply invoke the command :ref:`clouderp.py
...
Build an clouderp module
====================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

clouderp modules can either add brand new business logic to an clouderp system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to clouderp's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in clouderp thus starts and ends with modules.

Composition of a module
-----------------------

An clouderp module can contain a number of elements:

Business objects
    declared as Python classes, these resources are automatically persisted
    by clouderp based on their configuration
...
are specified by using the :option:`--addons-path <clouderp.py --addons-path>`
option.
...
An clouderp module is declared by its :ref:`manifest <reference/module/manifest>`.
...
clouderp provides a mechanism to help set up a new module, :ref:`clouderp.py
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ clouderp.py scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    install it in clouderp.

    .. only:: solutions

        #. Invoke the command ``clouderp.py scaffold openacademy addons``.
...
A key component of clouderp is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~gce.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~gce.models.Model._name` which is required and defines the name for
the model in the clouderp system. Here is a minimally complete definition of a
model::
...
:attr:`~gce.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~gce.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~gce.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~gce.fields.Field.index` (``bool``, default: ``False``)
    Requests that clouderp create a `database index`_ on the column
...
Example of simple fields are :class:`~gce.fields.Boolean`,
:class:`~gce.fields.Date`, :class:`~gce.fields.Char`.

Reserved fields
###############

clouderp creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~gce.fields.Model.id` (:class:`~gce.fields.Id`)
    the unique identifier for a record in its model
:attr:`~gce.fields.Model.create_date` (:class:`~gce.fields.Datetime`)
    creation date of the record
:attr:`~gce.fields.Model.create_uid` (:class:`~gce.fields.Many2one`)
    user who created the record
:attr:`~gce.fields.Model.write_date` (:class:`~gce.fields.Datetime`)
    last modification date of the record
:attr:`~gce.fields.Model.write_uid` (:class:`~gce.fields.Many2one`)
    user who last modified the record

Special fields
##############

By default, clouderp also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~gce.models.Model._rec_name`.
...
clouderp is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into clouderp
    :class: aphorism
...
* ``model`` is the name of the clouderp model for the record
...
.. exercise:: Define new menu entries
    ...
    .. only:: solutions
        ...
        #. Add it to the ``data`` list of ``openacademy/__gce__.py``
...
If no search view exists for the model, clouderp generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <gce.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <gce.fields.One2many>`
    A virtual relationship, inverse of a :class:`~gce.fields.Many2one`.
    A :class:`~gce.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~gce.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~gce.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <gce.fields.Many2many>`
...
clouderp provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~gce.models.Model._inherit`
    * :attr:`~gce.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), clouderp
...
In clouderp, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~gce.fields.Field.compute` to the name of a method. The computation
...
.. code-block:: python
    ...
    import random
    from gce import models, fields, api
...
on the compute method with the decorator :func:`~gce.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            clouderp has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~gce.api.onchange`
...
clouderp provides two ways to set up automatically verified invariants:
:func:`Python constraints <gce.api.constrains>` and
:attr:`SQL constraints <gce.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~gce.api.constrains`, and invoked on a recordset. The decorator
...
raise an exception if its invariant is not satisfied::
    ...
    from gce.exceptions import ValidationError
...
:attr:`~gce.models.Model._sql_constraints`. The latter is assigned to a list
...
Workflows may be associated with any object in clouderp, and are entirely
...
.. exercise:: Add access control through the gce interface
...
.. exercise:: Add access control through data files in your module
    ...
    * Make *Session* and *Course* readable by all users
    ...
    .. only:: solutions
        ...
        #. Finally update ``openacademy/__gce__.py`` to add the new data
           files to it
...
the same convention as the method :meth:`~gce.models.Model.write` of the ORM.
...
:class:`~gce.models.TransientModel` instead of
:class:`~gce.models.Model`. The class
:class:`~gce.models.TransientModel` extends :class:`~gce.models.Model`
...
pt_BR.po). Translations will be loaded automatically by clouderp for all
enabled languages. Developers always use English when creating a module, then
export the module terms using clouderp's gettext POT export feature
...
.. tip:: The Portable Object files generated by clouderp are published on
         `Transifex <https://www.transifex.com/clouderp/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from clouderp)
...
.. tip:: 
   ...
   By default clouderp's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`gce._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your clouderp installation. Translate your
   module using the facilities provided by clouderp.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``gce._`` and mark missing strings as translatable
...
clouderp 8.0 comes with a new report engine based on :ref:`reference/qweb`,
...
.. exercise:: Define a Dashboard
   ...
   .. only:: solutions
        ...
        #. Update ``openacademy/__gce__.py`` to reference the new data
           file
...
clouderp is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python program that interacts with an clouderp
...
.. exercise:: Add a new service to the client
   ...
   clouderp (yours, or your instructor's). This program should display all
...
The following example is a Python program that interacts with an clouderp server
