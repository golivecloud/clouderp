:banner: banners/installing_clouderp.jpg
...
There are mutliple ways to install clouderp, or not install it at all, depending
...
:ref:`setup/install/demo`
    ...
    clouderp or trying something out
:ref:`setup/install/saas`
    Trivial to start with and fully managed and migrated by clouderp S.A., can be
    used to both test clouderp and use it for your business, prevents complex
    customization (i.e. incompatible with custom modules or the clouderp Apps Store).

    Can be used for both testing clouderp and long-term production use.
:ref:`setup/install/packaged`
    ...
    Suitable for testing clouderp, developing modules and can be used for
...
:ref:`setup/install/source`
    ...
    multiple running clouderp versions on the same system, and don't provide easy
    source access to clouderp itself.
    ...
    or contribute.
`docker image <https://registry.hub.docker.com/_/clouderp/>`_
...
There are two different Editions_ of clouderp: the Community and Enterprise versions.
...
To simply get a quick idea of clouderp, demo_ instances are available. They are
...
clouderp's SaaS_ provides private instances and starts out free. It can be used to
discover and test clouderp and do non-code customizations without having to
install it locally.
...
clouderp provides packaged installers for Windows, deb-based distributions
...
clouderp will automatically be started at the end of the installation.

Configuration
'''''''''''''

The :ref:`configuration file <reference/cmdline/config>` can be found at
:file:`{%PROGRAMFILES%}\\clouderp 9.0-{id}\\server\\gce-server.conf`.

The configuration file can be edited to connect to a remote Postgresql, edit
file locations or set a dbfilter.

To reload the configuration file, restart the clouderp service via
:menuselection:`Services --> clouderp server`.
...
To install clouderp 9.0 Community on Debian-based distribution, execute the following
commands as root:

.. code-block:: console

    # wget -O - https://nightly.golive.pt/clouderp.key | apt-key add -
    # echo "deb http://nightly.golive.pt/9.0/nightly/deb/ ./" >> /etc/apt/sources.list
    # apt-get update && apt-get install clouderp
...
For clouderp 9.0 Enterprise, get the package from the Download_ page. You can then
use ``gdebi``:
...
This will install clouderp as a service, create the necessary PostgreSQL_ user
...
The :ref:`configuration file <reference/cmdline/config>` can be found at
:file:`/etc/clouderp/gce-server.conf`

When the configuration file is edited, clouderp must be restarted using
``service``:

.. code-block:: console

    $ sudo service clouderp restart
    Restarting clouderp: ok
...
.. warning::
    ...
    be added to the distribution's repositories for all of clouderp's
...
Execute the following commands to install clouderp 9.0 Community on your server:
...
.. code-block:: console
    ...
    $ sudo yum-config-manager --add-repo=https://nightly.golive.pt/9.0/nightly/rpm/clouderp.repo
    $ sudo yum install -y clouderp
    $ sudo systemctl enable clouderp
    $ sudo systemctl start clouderp

Enterprise
''''''''''

To install clouderp 9.0 Enterprise, execute these commands:
...
When the configuration file is edited, clouderp must be restarted via SystemD:

.. code-block:: console

    $ sudo systemctl restart clouderp
...
The source "installation" really is about not installing clouderp, and running
it directly from source instead.

This can be more convenient for module developers as the clouderp source is
...
It also makes starting and stopping clouderp more flexible and explicit than the
...
easily keeping (and running) multiple versions of clouderp side-by-side.

Community
---------

There are two way to get the clouderp source source zip or git.

* clouderp zip can be downloaded from  our nightly_ server or our Download_  page,
...
* git allows simpler update and easier switching between different versions
  of clouderp. It also simplifies maintaining non-module patches and
  contributions.  The primary drawback of git is that it is significantly
  larger than a tarball as it contains the entire history of the clouderp project.

  The git repository is https://github.com/golivecloud/clouderp.git for the Community
  version.
  ...
  .. code-block:: console
      ...
      $ git clone https://github.com/golivecloud/clouderp.git
...
.. code-block:: console
  ...
  $ git clone https://github.com/golivecloud/enterprise.git

If you use git_, you must modify the :option:`--addons-path <clouderp.py --addons-path>`
...
.. code-block:: console
  ...
  $ clouderp.py --addons-path=~/src/custom_modules,~/src/enterprise,~/src/clouderp/addons

.. warning:: The Enterprise git repository **does not contain the full clouderp
    source code**. You need to clone both the Community and Enterprise repository to
    have a working clouderp installation. The Download_ page contains the entire
...
* PostgreSQL, to use a local database
  ...
  only user is ``postgres``, and clouderp forbids connecting as ``postgres``.
  ...
  - on Windows, use `PostgreSQL for windows`_ then
    ...
    - create a postgres user with a password using the pg admin gui: open
      ...
      usename in the :guilabel:`Role Name` field (e.g. ``clouderp``), then open
      the :guilabel:`Definition` tab and enter the password (e.g. ``clouderp``),
      then click :guilabel:`OK`.

      The user and password must be passed to clouderp using either the
      :option:`-w <clouderp.py -w>` and :option:`-r <clouderp.py -r>` options or
...
* Python dependencies listed in the :file:`requirements.txt` file.
  ...
  - on Windows you need to install some of the dependencies manually, tweak the
    ...
    command from a cmd.exe prompt (replace ``\YourclouderpPath`` by the actual
    path where you downloaded clouderp):

    .. code-block:: ps1

        C:\> cd \YourclouderpPath
        C:\YourclouderpPath> C:\Python27\Scripts\pip.exe install -r requirements.txt
...
Once all dependencies are set up, clouderp can be launched by running ``clouderp.py``.
...
* PostgreSQL host, port, user and password.
  ...
  clouderp has no defaults beyond
...
Under Windows a typical way to execute clouderp would be:

.. code-block:: ps1

    C:\YourclouderpPath> python clouderp.py -w clouderp -r clouderp --addons-path=addons,../mymodules --db-filter=mydb$

Where ``clouderp``, ``clouderp`` are the postgresql login and password,
``../mymodules`` a directory with additional addons and ``mydb`` the default
db to serve on localhost:8069

Under Unix a typical way to execute clouderp would be:

.. code-block:: console

    $ ./clouderp.py --addons-path=addons,../mymodules --db-filter=mydb$

Where ``../mymodules`` is a directory with additional addons and ``mydb`` the
default db to serve on localhost:8069

.. _demo: https://demo.golive.pt
.. _docker: https://www.docker.com
.. _Download: https://www.golive.pt/page/download
...
.. _saas: https://www.golive.pt/page/start
...
.. _the repository: https://github.com/golivecloud/clouderp
.. _git: http://git-scm.com
.. _Editions: https://www.golive.pt/pricing#pricing_table_features
.. _nightly: https://nightly.golive.pt/9.0/nightly/
.. _extra: https://nightly.golive.pt/extra/
