...
def check_object_name(name):
    """ Check if the given name is a valid gce object name.
...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            # The (gce) module name can be in the ``gce.addons`` namespace
            # or not.  For instance, module ``sale`` can be imported as
            # ``gce.addons.sale`` (the right way) or ``sale`` (for backward
            # compatibility).
            module_parts = self.__module__.split('.')
            if len(module_parts) > 2 and module_parts[:2] == ['gce', 'addons']:
...
class BaseModel(object):
    """ Base class for gce models.

    gce models are created by inheriting from this class' subclasses:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _add_fake_fields(self, cr, uid, fields, context=None):
        from gce.fields import Char, Integer
    ...
    def _read_group_fill_results(self, cr, uid, domain, groupby, remaining_groupbys,
        ...
        all_groups,folded = self._group_by_full[groupby](self, cr, uid, present_group_ids, domain,
                                                  ...
                                                  access_rights_uid=gce.SUPERUSER_ID,
                                                  context=context)
                            ...
                            if f.required:
                                try:
                                    ...
                                    msg = "WARNING: unable to set column %s of table %s not null !\n"\
                                        "Try to re-run: gce-server --update=module\n"\
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~gce.fields.Integer`,
          :class:`~gce.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~gce.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~gce.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~gce.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~gce.fields.Date` and
              :class:`~gce.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`gce.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`gce.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _gce/models/relationals/format:

          :class:`~gce.fields.One2many` and
          :class:`~gce.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~gce.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~gce.fields.One2many`.
          ``(5, _, _)``
    ...
    def _where_calc(self, cr, user, domain, active_test=True, context=None):
        """Computes the WHERE clause needed to implement an gce domain.
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~gce.api.Environment`
        """
...
class Model(BaseModel):
    """Main super-class for regular database-persisted gce models.

    gce models are created by inheriting from this class::
