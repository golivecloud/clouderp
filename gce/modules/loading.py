...
import gce
import gce.modules.db
import gce.modules.graph
import gce.modules.migration
import gce.modules.registry
import gce.tools as tools
from gce import SUPERUSER_ID

from gce.tools.translate import _
from gce.modules.module import initialize_sys_path, \
    load_gce_module, init_module_models, adapt_version
from module import runs_post_install

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('gce.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
    ...
    def load_test(module_name, idref, mode):
        ...
        try:
            ...
            if tools.config.options['test_commit']:
                ...
                gce.modules.registry.RegistryManager.clear_caches(cr.dbname)
    ...
    processed_modules = []
    loaded_modules = []
    registry = gce.registry(cr.dbname)
    migrations = gce.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph):
        ...
        load_gce_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['gce.addons.%s' % (module_name,)]
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            modobj.update_translations(cr, SUPERUSER_ID, [module_id], None, {'overwrite': gce.tools.config["overwrite_existing_translations"]})
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(gce.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, gce.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    cr = db.cursor()
    try:
        if not gce.modules.db.is_initialized(cr):
        ...
        # gce.modules.registry.RegistryManager.new().
        registry = gce.registry(cr.dbname)
        ...
        if processed_modules:
            ...
            for (model, name) in cr.fetchall():
                if model in registry and not registry[model].is_transient() and not isinstance(registry[model], gce.osv.orm.AbstractModel):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['gce.addons.%s' % (pkg.name,)]
                ...
                gce.api.Environment.reset()
                return gce.modules.registry.RegistryManager.new(cr.dbname, force_demo, status, update_module)
        ...
        t0 = time.time()
        t0_sql = gce.sql_db.sql_counter
        if gce.tools.config['test_enable']:
            if update_module:
                cr.execute("SELECT name FROM ir_module_module WHERE state='installed' and name = ANY(%s)", (processed_modules,))
            else:
                cr.execute("SELECT name FROM ir_module_module WHERE state='installed'")
            for module_name in cr.fetchall():
                report.record_result(gce.modules.module.run_unit_tests(module_name[0], cr.dbname, position=runs_post_install))
            _logger.log(25, "All post-tested in %.2fs, %s queries", time.time() - t0, gce.sql_db.sql_counter - t0_sql)
    finally:
        cr.close()
