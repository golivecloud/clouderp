...
from gce.tools import assertion_report, lazy_property, classproperty, config, topological_sort
from gce.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __init__(self, db_name):
        ...
        self._db = gce.sql_db.db_connect(db_name)
        ...
        # Inter-process signaling (used only when gce.multi_process is True):
        ...
        cr = self.cursor()
        has_unaccent = gce.modules.db.has_unaccent(cr)
        if gce.tools.config['unaccent'] and not has_unaccent:
            _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
        self.has_unaccent = gce.tools.config['unaccent'] and has_unaccent
        cr.close()
...
class RegistryManager(object):
        ...
        with cls.lock():
            with gce.api.Environment.manage():
                ...
                try:
                    ...
                    gce.modules.load_modules(registry._db, force_demo, status, update_module)
                except Exception:
    ...
    def check_registry_signaling(cls, db_name):
        ...
        """
        changed = False
        if gce.multi_process and db_name in cls.registries:
