...
class AddonsImportHook(object):
    """
    Import hook to load gce addons from multiple paths.

    gce implements its own import-hook to load its addons. gce
    ...
    are living in `gce.addons`. The good way to import such modules is
    thus `import gce.addons.module`.
    """

    def find_module(self, module_name, package_path=None):
        module_parts = module_name.split('.')
        if len(module_parts) == 3 and module_name.startswith('gce.addons.'):
    ...
    def load_module(self, module_name):
        ...
        mod = imp.load_module('gce.addons.' + module_part, f, path, descr)
        sys.modules['gce.addons.' + module_part] = mod
        return mod

def initialize_sys_path():
    """
    Setup an import-hook to be able to import gce addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import gce.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
...
def get_module_filetree(module, dir='.'):
    ...
    files = gce.tools.osutil.listdir(path, True)
...
def init_module_models(cr, module_name, obj_list):
    ...
    for t in todo:
        t[1](cr, *t[2])
    if obj_list:
        obj_list[0].recompute(cr, gce.SUPERUSER_ID, {})
    cr.commit()

def load_gce_module(module_name):
    """ Load an gce module, if not already loaded.
    ...
    try:
        ...
        __import__('gce.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['gce.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='gce.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, gce.sql_db.sql_counter - t0_sql)
