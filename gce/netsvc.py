...
def LocalService(name):
    """
    The gce.netsvc.LocalService() function is deprecated. It still works
    in two cases: workflows and reports. For workflows, instead of using
    LocalService('workflow'), gce.workflow should be used (better yet,
    methods on gce.osv.orm.Model should be used). For reports,
    gce.report.render_report() should be used (methods on the Model should
    be provided too in the future).
    """
    assert gce.conf.deprecation.allow_local_service
    ...
    if name.startswith('report.'):
        report = gce.report.interface.report_int._reports.get(name)
        if report:
            return report
        else:
...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('gce.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
            cr.autocommit(True)
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running gce
...
DEFAULT_LOG_CONFIGURATION = [
    'gce.workflow.workitem:WARNING',
    'gce.http.rpc.request:INFO',
    'gce.http.rpc.response:INFO',
    'gce.addons.web.http:INFO',
    'gce.sql_db:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['gce:DEBUG','gce.http.rpc.request:DEBUG', 'gce.http.rpc.response:DEBUG'],
    'debug_rpc': ['gce:DEBUG','gce.http.rpc.request:DEBUG'],
    'debug': ['gce:DEBUG'],
    'debug_sql': ['gce.sql_db:DEBUG'],
    'info': [],
    'warn': ['gce:WARNING', 'werkzeug:WARNING'],
    'error': ['gce:ERROR', 'werkzeug:ERROR'],
    'critical': ['gce:CRITICAL', 'werkzeug:CRITICAL'],
}
