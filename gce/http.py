...
import gce
from gce.service.server import memory_info
from gce.service import security, model as service_model
from gce.tools.func import lazy_property
from gce.tools import ustr, consteq
...
# *broken*
NO_POSTMORTEM = (gce.osv.orm.except_orm,
                 gce.exceptions.AccessError,
                 gce.exceptions.ValidationError,
                 gce.exceptions.MissingError,
                 gce.exceptions.AccessDenied,
                 gce.exceptions.Warning,
                 gce.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                gce.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = gce.service.common.dispatch
        elif service_name == 'db':
            dispatch = gce.service.db.dispatch
        elif service_name == 'object':
            dispatch = gce.service.model.dispatch
        elif service_name == 'report':
            dispatch = gce.service.report.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                gce.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                gce.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except gce.exceptions.DeferredException, e:
        _logger.exception(gce.tools.exception_to_unicode(e))
        gce.tools.debugger.post_mortem(gce.tools.config, e.traceback)
        raise
    except Exception, e:
        _logger.exception(gce.tools.exception_to_unicode(e))
        gce.tools.debugger.post_mortem(gce.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all clouderp Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in gce.service.wsgi_server.application
        if self.db:
    ...
    def session(self):
        """
        a :class:`gceSession` holding the HTTP session data for the
    ...
    def cr(self):
        """
        :class:`~gce.sql_db.Cursor` initialized for the current method
        call.
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            gce.tools.debugger.post_mortem(
                gce.tools.config, sys.exc_info())
        raise
    ...
    def session_id(self):
        """
        opaque identifier for the :class:`gceSession` instance of
        the current request
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return gce.modules.registry.RegistryManager.get(self.db) if self.db else None
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        clouderp implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <gce.http.WebRequest.csrf_token`, the
          :data:`~gce.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          clouderp) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (gce.exceptions.Warning, SessionExpiredException, gce.exceptions.except_orm)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "clouderp Server Error",
                    'data': serialize_exception(exception)
            }
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "clouderp Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "clouderp Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, gce.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, gce.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, gce.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, gce.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, gce.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, gce.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, gce.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, gce.exceptions.except_orm):
...
def jsonrequest(f):
    """ 
        .. deprecated:: 8.0
            Use the :func:`~gce.http.route` decorator instead.
    """
...
clouderp URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.golive.pt/documentation/9.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through clouderp via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["gce", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('gce.addons.') and c.__module__.split(".")[2] in modules
...
class gceSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(gceSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ gce provides languages which may not make sense and/or may not
    ...
    def assert_valid(self, force=False):
        ...
        Ensures this session is valid (logged into the gce server)
        """
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        uid = self.uid or request.uid or gce.SUPERUSER_ID
...
class Root(object):
    """Root WSGI application for the gce Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = gce.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(path, session_class=gceSession)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + gce.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        for addons_path in gce.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    manifest_path = os.path.join(addons_path, module, '__gce__.py')
                    ...
                    if os.path.isfile(manifest_path) and os.path.isdir(path_static):
                        ...
                        if 'gce.addons' in sys.modules:
                            m = __import__('gce.addons.' + module)
                        else:
                            m = None
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-gce-Session-Id")
        if not sid:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    gce.modules.registry.RegistryManager.check_registry_signaling(db)
                    try:
                        with gce.tools.mute_logger('gce.sql_db'):
                        ...
                        gce.modules.registry.RegistryManager.signal_caches_change(db)
                else:
                    result = _dispatch_nodb()
...
def db_list(force=False, httprequest=None):
    dbs = gce.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    r = gce.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = gce.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
class CommonController(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact gce. """
