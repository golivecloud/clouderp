...
"""
gce - Server
gce is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - gce SA
"""
...
__author__ = gce.release.author
__version__ = gce.release.version

# Also use the `gce` logger for the main script.
_logger = logging.getLogger('gce')
...
def report_configuration():
    ...
    """
    config = gce.tools.config
    _logger.info("gce version %s", __version__)
    _logger.info('addons paths: %s', gce.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = gce.tools.config
    if not gce.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "w") as buf:
        registry = gce.modules.registry.RegistryManager.new(dbname)
        with gce.api.Environment.manage():
            with registry.cursor() as cr:
                gce.tools.trans_export(config["language"],
...
def main(args):
    ...
    if config["db_name"]:
        try:
            gce.service.db._create_empty_database(config["db_name"])
        except gce.service.db.DatabaseExists:
            pass
    ...
    setup_pid_file()
    rc = gce.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the clouderp server (default command)"""
    def run(self, args):
        main(args)
