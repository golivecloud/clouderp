# -*- coding: utf-8 -*-
"""
The module :mod:`gce.tests.common` provides unittest test cases and a few
...
# The gce library is supposed already configured.
ADDONS_PATH = gce.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = gce.tools.config['xmlrpc_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = gce.SUPERUSER_ID


def get_db_name():
    db = gce.tools.config['db_name']
...
class BaseCase(unittest.TestCase):
    """
    Subclass of TestCase for common gce-specific code.
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~gce.models.BaseModel`
        """
        assert "." in xid, "this method requires a fully qualified parameter, in the following form: 'module.identifier'"
        module, xid = xid.split('.')
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.cr = self.cursor()
        self.uid = gce.SUPERUSER_ID
        #: :class:`~gce.api.Environment` for the current test case
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = gce.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        gce.http.root.session_store.save(self.session)
    ...
    def authenticate(self, user, password):
        ...
        # gceSession.authenticate accesses the current request, which we
        ...
        gce.http.root.session_store.save(session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('gce.service.http.request.'):
