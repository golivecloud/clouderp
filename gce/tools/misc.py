...
"""
Miscellaneous tools used by gce.
"""
...
from gce.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if gce.tools.config['db_host']:
        env['PGHOST'] = gce.tools.config['db_host']
    if gce.tools.config['db_port']:
        env['PGPORT'] = str(gce.tools.config['db_port'])
    if gce.tools.config['db_user']:
        env['PGUSER'] = gce.tools.config['db_user']
    if gce.tools.config['db_password']:
        env['PGPASSWORD'] = gce.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the gce root, using a subdir folder.
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the gce root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by gce for translation
...
class upload_data_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fp = urllib.urlopen('http://www.gce.com/scripts/survey.php', args)
            fp.read()
            fp.close()
        except Exception:
            pass
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating gce domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('gce.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('gce.foo.bar'):
            do_suff()
