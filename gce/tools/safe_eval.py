...
gce domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an gce domain expression from an untrusted source.
    ...
    try:
    ...
    except gce.exceptions.except_orm:
        raise
    except gce.exceptions.Warning:
        raise
    except gce.exceptions.RedirectWarning:
        raise
        ...
        raise
    except gce.exceptions.MissingError:
        raise
    except Exception, e:
        import sys
        exc_info = sys.exc_info()
