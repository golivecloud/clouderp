...
def _get_idref(self, cr, uid, model_str, context, idref):
    idref2 = dict(idref,
                  ...
                  version=gce.release.major_version,
                  ref=_ref(self, cr),
                  pytz=pytz)
    if len(model_str):
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('gce.tools.convert.init').error(
        ...
        data = node.text
        if node.get('file'):
            with gce.tools.file_open(node.get('file'), 'rb') as f:
                data = f.read()
...
form: module.record_id""" % (xml_id,)
    ...
    def parse(self, de, mode=None):
        roots = ['gce','data','clouderp']
        if de.tag not in roots:
            raise Exception("Root xml tag must be <gce>, <clouderp> or <data>.")
        for rec in de:
            if rec.tag in roots:
                self.parse(rec, mode)
