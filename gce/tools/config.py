#gce.loggers.handlers. -*- coding: utf-8 -*-
...
import gce
import gce.conf
import gce.loglevels as loglevels
import logging
import gce.release as release
import appdirs
...
def _deduplicate_loggers(loggers):
    ...
    if for some odd reason people use :option`clouderp.py --save`` all the time.
    """
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.gce.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.gce_serverrc")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store clouderp data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "gce.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="gce.http.rpc.request:DEBUG", help='shortcut for --log-handler=gce.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="gce.http.rpc.response:DEBUG", help='shortcut for --log-handler=gce.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="gce.http:DEBUG", help='shortcut for --log-handler=gce.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="gce.sql_db:DEBUG", help='shortcut for --log-handler=gce.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate clouderp to another language."
    ...
    def parse_config(self, args=None):
        ...
        This method initializes gce.tools.config and gce.conf (the
        ...
        Typical usage of this method:
            ...
            gce.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        gce.netsvc.init_logger()
        gce.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'gce-server.conf')
        else:
            rcfilepath = os.path.expanduser('~/.gce_serverrc')

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('gce_SERVER') or rcfilepath)
        self.load()
        ...
        self.options['root_path'] = os.path.abspath(os.path.expanduser(os.path.expandvars(os.path.dirname(gce.__file__))))
        ...
        gce.conf.addons_paths = self.options['addons_path'].split(',')
        if opt.server_wide_modules:
            gce.conf.server_wide_modules = map(lambda m: m.strip(), opt.server_wide_modules.split(','))
        else:
            gce.conf.server_wide_modules = ['web','web_kanban']
    ...
    def _is_addons_path(self, path):
        for f in os.listdir(path):
            ...
            if os.path.isdir(modpath):
                ...
                if hasfile('__init__.py') and (hasfile('__gce__.py') or hasfile('__terp__.py')):
                    return True
        return False
