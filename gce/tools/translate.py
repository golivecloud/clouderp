...
WEB_TRANSLATION_COMMENT = "gce-web"
...
def trans_generate(lang, modules, cr):
    ...
    path_list = [(path, True) for path in gce.modules.module.ad_paths]
        ...
        for root, dummy, files in osutil.walksymlinks(path):
            ...
            if fnmatch.fnmatch(root, '*/static/src/xml*'):
                for fname in fnmatch.filter(files, '*.xml'):
                    babel_extract_terms(fname, path, root, 'gce.tools.translate:babel_extract_qweb',
                                        extra_comments=[WEB_TRANSLATION_COMMENT])
            if not recursive:
