...
class CreatorCase(common.TransactionCase):
    ...
    def make(self, value):
        id = self.model.create(self.cr, gce.SUPERUSER_ID, {'value': value})
        return self.model.browse(self.cr, gce.SUPERUSER_ID, [id])[0]
...
class test_integer_field(CreatorCase):
    ...
    def test_empty(self):
        self.assertEqual(self.model.search(self.cr, gce.SUPERUSER_ID, []), [],
...
class test_m2o(CreatorCase):
    ...
    def test_basic(self):
        ...
        name = dict(self.registry('export.integer').name_get(
            self.cr, gce.SUPERUSER_ID,[integer_id]))[integer_id]
        self.assertEqual(
...
class test_o2m(CreatorCase):
    ...
    def test_multiple_records_id(self):
        ...
        ids = O2M_c.browse(self.cr, gce.SUPERUSER_ID,
                           O2M_c.search(self.cr, gce.SUPERUSER_ID, []))
        self.assertEqual(
            export,
            [
                ['4', str(ids[0].id)],
...
class test_o2m_multiple(CreatorCase):
    ...
    def make(self, value=None, **values):
        ...
        id = self.model.create(self.cr, gce.SUPERUSER_ID, values)
        return self.model.browse(self.cr, gce.SUPERUSER_ID, [id])[0]
