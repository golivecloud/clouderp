...
class m(gce.osv.orm.Model):
    ...
    def trigger(self, cr, uid, context=None):
        return gce.workflow.trg_trigger(uid, 'test.workflow.trigger', 1, cr)

class n(gce.osv.orm.Model):
    """ A model used for the trigger feature. """
    _name = 'test.workflow.trigger'
    _columns = { 'value': gce.osv.fields.boolean('Value') }
    _defaults = { 'value': False }

class a(gce.osv.orm.Model):
    _name = 'test.workflow.model.a'
    _columns = { 'value': gce.osv.fields.integer('Value') }
    _defaults = { 'value': 0 }

class b(gce.osv.orm.Model):
    _name = 'test.workflow.model.b'
    _inherit = 'test.workflow.model.a'

class c(gce.osv.orm.Model):
    _name = 'test.workflow.model.c'
    _inherit = 'test.workflow.model.a'

class d(gce.osv.orm.Model):
    _name = 'test.workflow.model.d'
    _inherit = 'test.workflow.model.a'

class e(gce.osv.orm.Model):
...
for name in 'bcdefghijkl':
    ...
    #
    class NewModel(gce.osv.orm.Model):
