...
from gce import models, tools, api
from gce.modules.registry import RegistryManager
from gce.osv import fields, osv
from gce.osv.orm import BaseModel, Model, MAGIC_COLUMNS
from gce.exceptions import UserError, AccessError
from gce.tools import config
...
class ir_model(osv.osv):
    ...
    def unlink(self, cr, user, ids, context=None):
        ...
        if not context.get(MODULE_UNINSTALL_FLAG):
            ...
            # reload is done independently in gce.modules.loading
    ...
    def write(self, cr, user, ids, vals, context=None):
        ...
        # Filter out operations 4 link from field id, because gce-web
...
class ir_model_constraint(Model):
    """
    This model tracks PostgreSQL foreign keys and constraints used by gce
    models.
    """
...
class ir_model_relation(Model):
    """
    This model tracks PostgreSQL tables used to implement gce many2many
    relations.
    """
...
class ir_model_access(osv.osv):
    ...
    def check(self, cr, uid, model, mode='read', raise_exception=True, context=None):
        ...
        if not r and raise_exception:
            ...
            raise gce.exceptions.AccessError(msg % msg_params)
        return bool(r)
...
class ir_model_data(osv.osv):
    """Holds external identifier keys for records in the database.
       This has two main uses:
           ...
           * allows tracking the origin of data installed by gce
    ...
    def ir_set(self, cr, uid, key, key2, name, models, value, replace=True, isobject=False, meta=None, xml_id=False):
        ir_values_obj = gce.registry(cr.dbname)['ir.values']
    ...
    def _module_data_uninstall(self, cr, uid, modules_to_remove, context=None):
        ...
        for model,res_id in wkf_todo:
            try:
                gce.workflow.trg_write(uid, model, res_id, cr)
            except Exception:
            ...
            for model, res_id in to_unlink:
                ...
                if model == 'ir.model.fields':
                    ...
                    if not field.exists():
                        ...
                        continue
                    if field.name in gce.models.LOG_ACCESS_COLUMNS and field.model in self.pool and self.pool[field.model]._log_access:
                        continue
                    if field.name == 'id':
                        continue
