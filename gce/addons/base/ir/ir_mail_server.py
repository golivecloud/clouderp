...
from gce.tools import html2text
...
from gce.loglevels import ustr
...
class ir_mail_server(osv.osv):
    ...
    def connect(self, host, port, user=None, password=None, encryption=False, smtp_debug=False):
        ...
        if encryption == 'ssl':
            if not 'SMTP_SSL' in smtplib.__all__:
                raise UserError(_("Your gce Server does not support SMTP-over-SSL. You could use STARTTLS instead."
    ...
    def _get_default_bounce_address(self, cr, uid, context=None):
        ...
        If "mail.catchall.alias" is not set it defaults to "postmaster-clouderp".
