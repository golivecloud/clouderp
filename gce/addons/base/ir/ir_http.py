...
from gce.http import request, STATIC_CACHE
from gce.modules.module import get_resource_path, get_module_path
...
class ir_http(osv.AbstractModel):
    ...
    def _auth_method_public(self):
        if not request.session.uid:
            dummy, request.uid = self.pool['ir.model.data'].get_object_reference(request.cr, gce.SUPERUSER_ID, 'base', 'public_user')
        else:
            request.uid = request.session.uid
    ...
    def _authenticate(self, auth_method='user'):
        try:
            if request.session.uid:
                ...
                except (gce.exceptions.AccessDenied, gce.http.SessionExpiredException):
                    # All other exceptions mean undetermined status (e.g. connection pool full),
                    # let them bubble up
                    request.session.logout(keep_db=True)
            if request.uid is None:
                getattr(self, "_auth_method_%s" % auth_method)()
        except (gce.exceptions.AccessDenied, gce.http.SessionExpiredException, werkzeug.exceptions.HTTPException):
            raise
        except Exception:
    ...
    def _serve_attachment(self):
        ...
        attach = self.pool['ir.attachment'].search_read(request.cr, gce.SUPERUSER_ID, domain, ['__last_update', 'datas', 'name', 'mimetype', 'checksum'], context=request.context)
        if attach:
    ...
    def _postprocess_args(self, arguments, rule):
        ...
        for name, arg in arguments.items():
            if isinstance(arg, orm.browse_record) and arg._uid is UID_PLACEHOLDER:
                ...
                try:
                    arg.exists()
                except gce.models.MissingError:
    ...
    def routing_map(self):
        if not hasattr(self, '_routing_map'):
            ...
            if gce.tools.config['test_enable']:
                installed.add(gce.modules.module.current_test)
            mods = [''] + gce.conf.server_wide_modules + sorted(installed)
            self._routing_map = http.routing_map(mods, False, converters=self._get_converters())

        return self._routing_map

    def content_disposition(self, filename):
        filename = gce.tools.ustr(filename)
    ...
    def binary_content(self, xmlid=None, model='ir.attachment', id=None, field='datas', unique=False, filename=None, filename_field='datas_fname', download=False, mimetype=None, default_mimetype='application/octet-stream', env=None):
        ...
        except gce.exceptions.AccessError:
            return (403, [], None)
