...
import gce.tools.lru
from gce.exceptions import QWebException
from gce.fields import Datetime
from gce.http import request
from gce.tools.safe_eval import safe_eval as eval
from gce.osv import osv, orm, fields
from gce.tools import html_escape as escape
from gce.tools.misc import find_in_path
from gce.tools.translate import _
from gce.modules.module import get_resource_path
...
class QWeb(orm.AbstractModel):
    ...
    * alternatively, override :meth:`~.get_converter_for` and return an
    ...
    def get_converter_for(self, field_type):
        """ returns a :class:`~gce.models.Model` used to render a
        ``t-field``.
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        ...
        if isinstance(value, basestring):
            value = datetime.datetime.strptime(
                value[:10], gce.tools.DEFAULT_SERVER_DATE_FORMAT)

        if options and 'format' in options:
            pattern = options['format']
        else:
            strftime_pattern = lang.date_format
            pattern = gce.tools.posix_to_ldml(strftime_pattern, locale=locale)
...
class DateTimeConverter(osv.AbstractModel):
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        ...
        if isinstance(value, basestring):
            value = datetime.datetime.strptime(
                value, gce.tools.DEFAULT_SERVER_DATETIME_FORMAT)
...
class RelativeDatetimeConverter(osv.AbstractModel):
    ...
    def value_to_html(self, cr, uid, value, field, options=None, context=None):
        parse_format = gce.tools.DEFAULT_SERVER_DATETIME_FORMAT
        locale = babel.Locale.parse(
            self.user_lang(cr, uid, context=context).code)
...
class AssetsBundle(object):
    ...
    def clean_attachments(self, type):
        ...
        attachment_ids = ira.search(self.cr, gce.SUPERUSER_ID, domain, context=self.context)
        return ira.unlink(self.cr, gce.SUPERUSER_ID, attachment_ids, context=self.context)
    ...
    def get_attachments(self, type, inc=None):
        ...
        attachment_ids = ira.search(self.cr, gce.SUPERUSER_ID, domain, order='name asc', context=self.context)
        return ira.browse(self.cr, gce.SUPERUSER_ID, attachment_ids, context=self.context)
    ...
    def save_attachment(self, type, content, inc=None):
        ...
        attachment_id = ira.create(self.cr, gce.SUPERUSER_ID, values, context=self.context)
        ...
        ira.write(self.cr, gce.SUPERUSER_ID, attachment_id, values, context=self.context)
        ...
        return ira.browse(self.cr, gce.SUPERUSER_ID, attachment_id, context=self.context)
                    ...
                    if debug:
                        try:
                            ...
                            with self.cr.savepoint():
                                ira.create(self.cr, gce.SUPERUSER_ID, dict(
    ...
    def stat(self):
        if not (self.inline or self._filename or self._ir_attach):
            ...
            try:
                ...
                ira = self.registry['ir.attachment']
                attach = ira.search_read(self.cr, gce.SUPERUSER_ID, domain, fields, context=self.context)
