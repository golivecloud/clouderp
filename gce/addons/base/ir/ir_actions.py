...
import gce.report.interface
from gce.report.report_sxw import report_sxw, report_rml
from gce.tools import ormcache
from gce.tools.safe_eval import safe_eval as eval
from gce.tools.translate import _
import gce.workflow
from gce.exceptions import MissingError, UserError
...
class ir_actions_report_xml(osv.osv):
    ...
    def _lookup_report(self, cr, name):
        ...
        if 'report.' + name in gce.report.interface.report_int._reports:
            new_report = gce.report.interface.report_int._reports['report.' + name]
        else:
            ...
            if r:
                if r['report_type'] in ['qweb-pdf', 'qweb-html']:
                    ...
                    if r['parser']:
                        kwargs = { 'parser': operator.attrgetter(r['parser'])(gce.addons) }
                    else:
                        kwargs = {}
    ...
    def render_report(self, cr, uid, res_ids, name, data, context=None):
        ...
        if isinstance(new_report, (str, unicode)):  # Qweb report
            ...
            if gce.tools.config['test_enable'] and not tools.config['test_report_directory']:
...
class ir_actions_act_window(osv.osv):
    ...
    def exists(self):
        ...
        if len(existing) < len(self):
            ...
            (self - existing)._cache.update(gce.fields.FailedValue(exc))
        return existing
...
class ir_actions_server(osv.osv):
    ...
    Since gce 8.0 a button 'Create Menu Action' button is available on the
...
# To return an action, assign: action = {...}""",
    ...
    def run_action_trigger(self, cr, uid, action, eval_context=None, context=None):
        ...
        if action.use_relational_model == 'relational':
            ...
            if not isinstance(record, gce.models.BaseModel):
    ...
    def _get_eval_context(self, cr, uid, action=None, context=None):
        ...
        env = gce.api.Environment(cr, uid, context)
        ...
        eval_context.update({
            ...
            'workflow': workflow,
            # Exceptions
            'Warning': gce.exceptions.Warning,
            # record
