...
from gce import api, tools
import gce.modules
from gce.osv import fields, osv
from gce.tools.translate import _
from gce.exceptions import AccessError, UserError, ValidationError
...
class ir_translation(osv.osv):
    ...
    _columns = {
        ...
        # aka gettext extracted-comments - we use them to flag gce-web translation
    ...
    def load_module_terms(self, cr, modules, langs, context=None):
        ...
        for module_name in modules:
            modpath = gce.modules.get_module_path(module_name)
            if not modpath:
                continue
            for lang in langs:
                ...
                if base_lang_code:
                    base_trans_file = gce.modules.get_module_resource(module_name, 'i18n', base_lang_code + '.po')
                    if base_trans_file:
                    ...
                    base_trans_extra_file = gce.modules.get_module_resource(module_name, 'i18n_extra', base_lang_code + '.po')
                ...
                trans_file = gce.modules.get_module_resource(module_name, 'i18n', lang_code + '.po')
                if trans_file:
                ...
                trans_extra_file = gce.modules.get_module_resource(module_name, 'i18n_extra', lang_code + '.po')
                if trans_extra_file:
