...
from gce import modules, tools
from gce.modules.db import create_categories
from gce.modules import get_module_resource
from gce.tools import ormcache
from gce.tools.parse_version import parse_version
from gce.tools.translate import _
from gce.tools import html_sanitize
from gce.osv import osv, orm, fields
from gce import api, fields as fields2
...
class module(osv.osv):
    ...
    _columns = {
        ...
        'license': fields.selection([
            ...
            ('OEEL-1', 'clouderp Enterprise Edition License v1.0'),
    ...
    def _button_immediate_function(self, cr, uid, ids, function, context=None):
        ...
        registry = gce.modules.registry.RegistryManager.new(cr.dbname, update_module=True)
    ...
    def install_from_urls(self, cr, uid, urls, context=None):
        ...
        gce = gce.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from gce Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == gce or not url:
                    continue    # gce is special case, handled below, and no URL means local module
            ...
            if urls.get(gce, None):
                ...
                # copy all modules in the SERVER/gce/addons directory to the new "gce" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, gce, 'addons', d)    # XXX 'gce' subdirectory ?
                        shutil.copytree(os.path.join(base_path, d), destdir)

                # then replace the server by the new "base" module
                server_dir = gce.tools.config['root_path']      # XXX or dirname()
                bck = backup(server_dir)
                _logger.info('Copy downloaded module `gce` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, gce), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self, cr, uid, context=None):
        return tools.config.get('apps_server', 'https://apps.gce.com/apps')
