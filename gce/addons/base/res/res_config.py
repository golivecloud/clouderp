...
class res_config_settings(osv.osv_memory, res_config_module_installation_mixin):
    ...
    def execute(self, cr, uid, ids, context=None):
        ...
        if not self.pool['res.users']._is_admin(cr, uid, [uid]):
            raise gce.exceptions.AccessError(_("Only administrators can change the settings"))
        ...
        res_config = gce.modules.registry.RegistryManager.get(cr.dbname)['res.config']
    ...
    def get_config_warning(self, cr, msg, context=None):
        ...
        Example of use:
        ---------------
        from gce.addons.base.res.res_config import get_warning_config
        ...
        res_config_obj = gce.registry(cr.dbname)['res.config.settings']
