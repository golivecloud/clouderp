...
import gce
from gce import api
from gce import SUPERUSER_ID, models
from gce import tools
import gce.exceptions
from gce import api
from gce.osv import fields, osv, expression
from gce.service.db import check_super
...
class res_users(osv.osv):
    """ User class. A res.users record models an gce user and is different
        from an employee.
    ...
    name = gce.fields.Char(related='partner_id.name', inherited=True)
    email = gce.fields.Char(related='partner_id.email', inherited=True)
    log_ids = gce.fields.One2many('res.users.log', 'create_uid', string='User log entries')
    login_date = gce.fields.Datetime(related='log_ids.create_date', string='Latest connection')
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False, lazy=True):
        if uid != SUPERUSER_ID:
            ...
            if groupby_fields.intersection(USER_PRIVATE_FIELDS):
                raise gce.exceptions.AccessError('Invalid groupby')
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        if user != SUPERUSER_ID and args:
            ...
            if domain_fields.intersection(USER_PRIVATE_FIELDS):
                raise gce.exceptions.AccessError('Invalid search criterion')
    ...
    def unlink(self, cr, uid, ids, context=None):
        if 1 in ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by clouderp (updates, module installation, ...)'))
        db = cr.dbname
        if db in self.__uid_cache:
            for id in ids:
                if id in self.__uid_cache[db]:
    ...
    def change_password(self, cr, uid, old_passwd, new_passwd, context=None):
        ...
        :return: True
        :raise: gce.exceptions.AccessDenied when old password is wrong
        :raise: gce.exceptions.UserError when new password is not set or empty
        """
    ...
    @api.multi
    def _is_admin(self):
        return self.id == gce.SUPERUSER_ID or self.sudo(self).has_group('base.group_erp_manager')
