...
from gce.osv.expression import get_unaccent_wrapper
...
class res_partner(osv.Model, format_address):
    ...
    _columns = {
        ...
        'company_type': fields.selection(
            ...
            help='Technical field, used only to display a boolean using a radio '
                 'button. As for clouderp v9 RadioButton cannot be used on boolean '
    ...
    image = gce.fields.Binary("Image", attachment=True,
        help="This field holds the image used as avatar for this contact, limited to 1024x1024px",
        default=lambda self: self._get_default_image(False, True))
    image_medium = gce.fields.Binary("Medium-sized image", attachment=True,
        help="Medium-sized image of this contact. It is automatically "\
             "resized as a 128x128px image, with aspect ratio preserved. "\
             "Use this field in form views or some kanban views.")
    image_small = gce.fields.Binary("Small-sized image", attachment=True,
    ...
    def _get_default_image(self, is_company, colorize=False):
        ...
        if self.env.context.get('partner_type') == 'delivery':
            img_path = gce.modules.get_module_resource('base', 'static/src/img', 'truck.png')
        elif self.env.context.get('partner_type') == 'invoice':
            img_path = gce.modules.get_module_resource('base', 'static/src/img', 'money.png')
        else:
            img_path = gce.modules.get_module_resource(
