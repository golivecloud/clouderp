...
def check_super(passwd):
    if passwd and passwd == gce.tools.config['admin_passwd']:
        return True
    raise gce.exceptions.AccessDenied()

# This should be moved to gce.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None):
    try:
        db = gce.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by RegistryManager.new().
            gce.modules.db.initialize(cr)
            gce.tools.config['lang'] = lang
            cr.commit()

        registry = gce.modules.registry.RegistryManager.new(
        ...
        with closing(db.cursor()) as cr:
            ...
            if login:
                values['login'] = login
                emails = gce.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = gce.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = gce.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    gce.sql_db.close_db(db_original_name)
    db = gce.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    from_fs = gce.tools.config.filestore(db_original_name)
    to_fs = gce.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    gce.modules.registry.RegistryManager.delete(db_name)
    gce.sql_db.close_db(db_name)
    ...
    fs = gce.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'clouderp_dump': '1',
        'db_name': cr.dbname,
        'version': gce.release.version,
        'version_info': gce.release.version_info,
        'major_version': gce.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with gce.tools.osutil.tempdir() as dump_dir:
            filestore = gce.tools.config.filestore(db_name)
            ...
            gce.tools.exec_pg_command(*cmd)
            if stream:
                gce.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                gce.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = gce.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with gce.tools.osutil.tempdir() as dump_dir:
        ...
        if gce.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = gce.modules.registry.RegistryManager.new(db)
...
def exp_rename(old_name, new_name):
    gce.modules.registry.RegistryManager.delete(old_name)
    gce.sql_db.close_db(old_name)
    ...
    old_fs = gce.tools.config.filestore(old_name)
    new_fs = gce.tools.config.filestore(new_name)
    if os.path.exists(old_fs) and not os.path.exists(new_fs):
        shutil.move(old_fs, new_fs)
    return True

def exp_change_admin_password(new_password):
    gce.tools.config['admin_passwd'] = new_password
    gce.tools.config.save()
    return True

def exp_migrate_databases(databases):
    for db in databases:
        _logger.info('migrate database %s', db)
        gce.tools.config['update']['base'] = True
        gce.modules.registry.RegistryManager.new(db, force_demo=False, update_module=True)
    return True
...
@gce.tools.mute_logger('gce.sql_db')
def exp_db_exist(db_name):
    ## Not True: in fact, check if connection to database is possible. The database may exists
    return bool(gce.sql_db.db_connect(db_name))

def list_dbs(force=False):
    if not gce.tools.config['list_db'] and not force:
        raise gce.exceptions.AccessDenied()
    chosen_template = gce.tools.config['db_template']
    ...
    with closing(db.cursor()) as cr:
        try:
            db_user = gce.tools.config["db_user"]
            if not db_user and os.name == 'posix':
                import pwd
                db_user = pwd.getpwuid(os.getuid())[0]
            if not db_user:
                cr.execute("select usename from pg_user where usesysid=(select datdba from pg_database where datname=%s)", (gce.tools.config["db_name"],))
                res = cr.fetchone()
            ...
            res = [gce.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
            res = []
    res.sort()
    return res
...
def exp_list(document=False):
    if not gce.tools.config['list_db']:
        raise gce.exceptions.AccessDenied()
    return list_dbs()

def exp_list_lang():
    return gce.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(gce.tools.config['root_path'], 'addons/base/res/res_country_data.xml')).getroot()
