...
import gce
from gce.exceptions import UserError, ValidationError, QWebException
from gce.tools.translate import translate
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
                ...
                try:
                    if args and len(args) > 1:
                        ...
                        if len(args) > 3 and isinstance(args[3], (long, int, list)):
                            ...
                            ids = []
                    cr = gce.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if gce.registry(dbname)._init and not gce.tools.config['test_enable']:
                    raise gce.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    object = gce.registry(cr.dbname).get(obj)
    if object is None:
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with gce.registry(db).cursor() as cr:
