...
from gce.release import nt_service_name
import gce.tools.config as config
from gce.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'gce.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in gce.modules.module.ad_paths:
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        while True:
            ...
            registries = gce.modules.registry.RegistryManager.registries
            _logger.debug('cron%d polling for jobs', number)
            for db_name, registry in registries.iteritems():
                while registry.ready:
                    acquired = gce.addons.base.ir.ir_cron.ir_cron._acquire_job(db_name)
                    if not acquired:
                        break
    ...
    def cron_spawn(self):
        ...
        for i in range(gce.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="gce.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="gce.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        gce.modules.registry.RegistryManager.delete_all()
        logging.shutdown()
...
class PreforkServer(CommonServer):
    ...
    def long_polling_spawn(self):
        ...
        cmd = nargs[0]
        cmd = os.path.join(os.path.dirname(cmd), "gce-gevent")
        nargs[0] = cmd
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('gce: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(gce.modules.registry.RegistryManager.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = gce.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('gce.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            import gce.addons.base as base
...
def load_server_wide_modules():
    for m in gce.conf.server_wide_modules:
        try:
            gce.modules.module.load_gce_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `gce-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute gce-server process with (nearly) the same arguments"""
    if gce.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        gce.tools.convert_yaml_import(cr, 'base', file(test_file), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in sys.modules.items():
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = gce.modules.module.TestStream()
...
def start(preload=None, stop=False):
    """ Start the gce http server and cron processor.
    """
    global server
    load_server_wide_modules()
    if gce.evented:
        server = GeventServer(gce.service.wsgi_server.application)
    elif config['workers']:
        server = PreforkServer(gce.service.wsgi_server.application)
    else:
        server = ThreadedServer(gce.service.wsgi_server.application)
