...
from gce.workflow.helpers import Session
from gce.workflow.helpers import Record
from gce.workflow.helpers import WorkflowActivity
...
class Environment(dict):
    ...
    def __init__(self, session, record):
        ...
        self.id = record.id
        self.ids = [record.id]
        self.obj = gce.registry(self.cr.dbname)[self.model]
...
class WorkflowItem(object):
    ...
    def wkf_expr_execute_action(self, activity):
        ...
        ir_actions_server = gce.registry(self.session.cr.dbname)['ir.actions.server']
    ...
    def wkf_expr_check(self, transition, signal):
        ...
        if self.session.uid != gce.SUPERUSER_ID and transition['group_id']:
            registry = gce.registry(self.session.cr.dbname)
